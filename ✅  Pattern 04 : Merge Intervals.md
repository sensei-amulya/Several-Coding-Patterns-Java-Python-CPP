...
public static Object merge (intervals) {
    ...
    for i in range(1, len(intervals)):
        ...
        if interval.start <= end:
            #overlapping intervals, adjust the end
                ...
                #non-overlapping intercal, add the precious interval and reset
                mergedIntervals.append(new Interval(start, end))
                start = interval.start
                end = interval.end
        #add the last interval
        mergedIntervals.append(new Interval(start, end))
...
auto merge (intervals) {
  ...
  //sort the intervals on the startTime
  intervals.sort((a,b) => a.start - b.start)
  auto mergedIntervals = []
  
  auto start = intervals[0].start
  ...
  for(auto i = 1; i < intervals.size(); i++) {
    ...
    if(interval.start <= end) {
      ...
      //non-overlapping intercal, add the precious interval and reset
      mergedIntervals.push_back(new Interval(start, end))
      start = interval.start
      end = interval.end
    }
  }
...
merged_intervals = merge([new Interval(1, 4), new Interval(2, 5), new Interval(7, 9)]);
result = "";
for(i=0; i < merged_intervals.size(); i++) {
  result += merged_intervals[i].get_interval() + " ";
}
...
auto merge(intervals) {
  ...
  for(auto i = 1; i < intervals.size(); i++) {
    ...
    auto previous = intervals[i-1]
    
    if(current[0] <= previous[1]) {
      intervals[i] = [previous[0], std::max(previous[1], current[1])]
      intervals.splice(i-1, 1)
      i--
    }
  }
...
public static Object insert (intervals, newInterval) {
  merged = [];
  i = 0
...
def insert(intervals, newInterval):
    ...
    #skip and add to output all intervals that come before the newInterval
    while i < len(intervals) && intervals[i].end < newInterval.start:
        merged.append(intervals[i])
        i++

    # merge all intervals that overlap with newInterval
    while i < len(intervals) && intervals[i].start <= newInterval.end:
...
auto insert (intervals, newInterval) {
  auto merged = [];
  auto i = 0
...
### Java
```java
public static Object findIntersection(firstIntervals, secondIntervals) {
  result = []
  ...
  while(i < firstIntervals.length && j < secondIntervals.length) {
    ...
    secondOverlapsFirst = secondIntervals[j][0] >= firstIntervals[i][0] && secondIntervals[j][0] <= firstIntervals[i][1]
    
    //store the intersection part
    if(firstOverlapsSecond || secondOverlapsFirst) {
      result.add([Math.max(firstIntervals[i][0], secondIntervals[j][0]), Math.min(firstIntervals[i][1], secondIntervals[j][1])])
    }
...
### Python
```python
def findIntersection(firstIntervals, secondIntervals):
    result = []

    i = 0
    j = 0

    while i < len(firstIntervals) && j < len(secondIntervals):
        #check if intervals overlap and firstIntervals[i] start time
        #lies within the other secondIntervals[j]
        firstOverlapsSecond = firstIntervals[i][0] >= secondIntervals[j][0] && firstIntervals[i][0] <= secondIntervals[j][1]
        ...
        #store the intersection part
        if firstOverlapsSecond || secondOverlapsFirst:
            result.append([max(firstIntervals[i][0], secondIntervals[j][0]), min(firstIntervals[i][1], secondIntervals[j][1])])
...
### C++
```cpp
auto findIntersection(firstIntervals, secondIntervals) {
  auto result = []
  ...
  while(i < firstIntervals.size() && j < secondIntervals.size()) {
    //check if intervals overlap and firstIntervals[i] start time
    //lies within the other secondIntervals[j]
    auto firstOverlapsSecond = firstIntervals[i][0] >= secondIntervals[j][0] && firstIntervals[i][0] <= secondIntervals[j][1]
    
    //check if intervals overlap and firstIntervals[j]'s start time 
    //lies within the other secondInterval[i]
    auto secondOverlapsFirst = secondIntervals[j][0] >= firstIntervals[i][0] && secondIntervals[j][0] <= firstIntervals[i][1]
    
    //store the intersection part
    if(firstOverlapsSecond || secondOverlapsFirst) {
      result.push_back([std::max(firstIntervals[i][0], secondIntervals[j][0]), std::min(firstIntervals[i][1], secondIntervals[j][1])])
    }
...
### Java
```java
public static Object canAttendAllAppointments(appointmentTimes) {
  //sort intervals by start time
  appointmentTimes.sort((a,b) => a[0] -b[0])
  
  //check if any two intervals overlap
  for(i = 1; i < appointmentTimes.length; i++) {
...
def canAttendAllAppointments(appointmentTimes):
    ...
    #check if any two intervals overlap
    for i in range(1, len(appointmentTimes)):
        if appointmentTimes[i][0] < appointmentTimes[i-1][1]:
            #note that in the comparison above, it is < and not <=
            #while merging we needed <= comparison, as we will be merging the two
            #intervals have conditions appointmentTimes[i][0] === appointmentTimes[i-1][1]
...
### C++
```cpp
auto canAttendAllAppointments(appointmentTimes) {
  //sort intervals by start time
  appointmentTimes.sort((a,b) => a[0] -b[0])
  
  //check if any two intervals overlap
  for(auto i = 1; i < appointmentTimes.size(); i++) {
    if(appointmentTimes[i][0] < appointmentTimes[i-1][1]) {
      //note that in the comparison above, it is < and not <=
      //while merging we needed <= comparison, as we will be merging the two 
      //intervals have conditions appointmentTimes[i][0] === appointmentTimes[i-1][1]
...
### Java
```java
public static Object whatAreTheConflicts(appointmentTimes)  {
  
  // appointmentTimes.sort((a,b) => a[0]-b[0])
  
  conflicts = []
  
  for(i = 0; i < appointmentTimes.length -1; i++) {
    for(j = 1; j < appointmentTimes.length; j++) {
      if((j!==i) && (appointmentTimes[i][1] > appointmentTimes[j][0])) {
        conflicts.add([appointmentTimes[j], appointmentTimes[i]])
      } 
    }
 }
...
### Python
```python
def whatAreTheConflicts(appointmentTimes):
    ...
    for i in range(len(appointmentTimes) -1):
        for j in range(1, len(appointmentTimes)):
            if (j!==i:&& (appointmentTimes[i][1] > appointmentTimes[j][0])):
                conflicts.append([appointmentTimes[j], appointmentTimes[i]])
...
### C++
```cpp
auto whatAreTheConflicts(appointmentTimes)  {
  
  // appointmentTimes.sort((a,b) => a[0]-b[0])
  
  auto conflicts = []
  
  for(auto i = 0; i < appointmentTimes.size() -1; i++) {
    for(auto j = 1; j < appointmentTimes.size(); j++) {
      if((j!==i) && (appointmentTimes[i][1] > appointmentTimes[j][0])) {
        conflicts.push_back([appointmentTimes[j], appointmentTimes[i]])
      } 
    }
 }
 // std::cout << appointmentTimes)
 return conflicts
}
...
public static Object minMeetingRooms(meetings) {
  ...
  public static Object getEarliest(room) {
  ...
  for(i = 1; i < meetings.length; i++) {
    earliestRoom = getEarliest(rooms)
...
auto minMeetingRooms(meetings) {
  ...
  auto rooms = [meetings[0]]
  
  for(auto i = 1; i < meetings.size(); i++) {
    auto earliestRoom = getEarliest(rooms)
    auto currentTime = meetings[i]
...
### Java
```java
public static Object findMaxCPULoad(jobs) {
...
def findMaxCPULoad(jobs):
    ...
    for i in range(len(jobs)):
        maxCPULoad = max(maxCPULoad, jobs[i][2])
...
auto findMaxCPULoad(jobs) {
 ...
 //consolidate jobs that overlap
  ...
  for(auto i = 0; i < jobs.size(); i++) {
    maxCPULoad = std::max(maxCPULoad, jobs[i][2]) 
  }
...
### Java
```java
public static Object findEmployeeFreeTime (schedules) {
  freeTime = [];
  ...
  for(i = 0; i < schedules.length; i++) {
    for(j = 0; j <  schedules[i].length; j++) {
      allTime.add(schedules[i][j])
    }
  }
  allTime.sort((a,b) => a[0]-b[0])

  //merge overlap
  for(i = 1; i < allTime.length; i++) {
    current = allTime[i]
    previous = allTime[i-1]
  ...
  for(i = 1; i < allTime.length; i++) {
    freeTime.add([allTime[i-1][1], allTime[i][0]])
  }
    return freeTime;
};
...
def findEmployeeFreeTime(schedules):
    ...
    for i in range(len(schedules)):
        for j in range(schedules[i].length):
            allTime.append(schedules[i][j])
    allTime.sort((a,b) => a[0]-b[0])
    ...
    #whatever is not accounted for is free time
    for i in range(1, len(allTime)):
        freeTime.append([allTime[i-1][1], allTime[i][0]])
    return freeTime
...
### C++
```cpp
auto findEmployeeFreeTime (schedules) {
  auto freeTime = [];
  ...
  for(auto i = 0; i < schedules.size(); i++) {
    for(auto j = 0; j <  schedules[i].size(); j++) {
      allTime.push_back(schedules[i][j])
    }
  }
  allTime.sort((a,b) => a[0]-b[0])

  //merge overlap
  for(auto i = 1; i < allTime.size(); i++) {
    auto current = allTime[i]
    auto previous = allTime[i-1]
  ...
  //whatever is not accounted for is free time
  for(auto i = 1; i < allTime.size(); i++) {
    freeTime.push_back([allTime[i-1][1], allTime[i][0]])
  }
    return freeTime;
};
