...
### Java
```java
public static Object binarySearch (arr, key) {
  start = 0
  end = arr.length -1
...
### C++
```cpp
auto binarySearch (arr, key) {
  auto start = 0
  auto end = arr.size() -1
  
  //check to see if arr is sorted ascending or descending
  auto isAscending = arr[start] < arr[end]
  
  while(start <= end) {
    //calculate the middle of the current range
    auto middle = Math.floor(start + (end-start)/2)
...
### Java
```java
public static Object searchCeilingOfNumber(arr, key) {
  n = arr.length 
  start = 0;
  end = n - 1;
...
### Python
```python
def searchCeilingOfNumber(arr, key):
    n = len(arr)
    start = 0
    end = n - 1
...
### C++
```cpp
auto searchCeilingOfNumber(arr, key) {
  auto n = arr.size() 
  auto start = 0;
  auto end = n - 1;
...
### Java
```java
public static Object searchFloorOfNumber(arr, key) {
  start = 0;
  end = arr.length - 1;
...
### Python
```python
def searchFloorOfNumber(arr, key):
    start = 0
    end = len(arr) - 1
...
### C++
```cpp
auto searchFloorOfNumber(arr, key) {
  auto start = 0;
  auto end = arr.size() - 1;
...
### Java
```java
public static Object searchNextLetter(letters, key) {
  n = letters.length;
  start = 0;
  end = n - 1;
...
### Python
```python
def searchNextLetter(letters, key):
    n = len(letters)
    start = 0
    end = n - 1
...
### C++
```cpp
auto searchNextLetter(letters, key) {
  auto n = letters.size();
  auto start = 0;
  auto end = n - 1;
...
### Java
```java
public static Object findRange(arr, key) {
  result = [-1, -1]
...
public static Object binarySearch(arr, key, findMaxIndex) {
  keyIndex = -1;
  start = 0;
  end = arr.length - 1;
...
def binarySearch(arr, key, findMaxIndex):
    keyIndex = -1
    start = 0
    end = len(arr) - 1
...
auto binarySearch(arr, key, findMaxIndex) {
  auto keyIndex = -1;
  auto start = 0;
  auto end = arr.size() - 1;
...
public static Object searchInInfiniteArray(reader, key) {
...
public static Object binarySearch(reader, key, start, end) {
  while (start <= end) {
    mid = Math.floor(start + (end - start) / 2);
...
class ArrayReader:
    ...
    get(index):
        if index >= this.len(arr):return Number.MAX_SAFE_INTEGER
        return this.arr[index]
...
class ArrayReader {
  ...
  get(index) {
    if (index >= this.arr.size()) return Number.MAX_SAFE_INTEGER;
    return this.arr[index];
  }
}

auto searchInInfiniteArray(reader, key) {
  //1. find the proper bounds
  auto start = 0;
  auto end = 1;
  while (reader.get(end) < key) {
...
auto binarySearch(reader, key, start, end) {
  while (start <= end) {
    auto mid = Math.floor(start + (end - start) / 2);
...
### Java
```java
public static Object searchMinDiffElement(arr, key) {
  start = 0;
  end = arr.length - 1;
...
### C++
```cpp
auto searchMinDiffElement(arr, key) {
  auto start = 0;
  auto end = arr.size() - 1;
...
### Java
```java
public static Object findMaxInBitonicArray(arr) {
  start = 0;
  end = arr.length - 1;
...
### C++
```cpp
auto findMaxInBitonicArray(arr) {
  auto start = 0;
  auto end = arr.size() - 1;
...
### Java
```java
public static Object searchBitonicArray(arr, key) {
...
public static Object findMax(arr) {
  start = 0;
  end = arr.length - 1;
...
public static Object binarySearch(arr, key, start, end) {
  //check to see if arr is sorted ascending or descending
  isAscending = arr[start] < arr[end];
...
### C++
```cpp
auto searchBitonicArray(arr, key) {
  auto maxIndex = findMax(arr);
  auto keyIndex = binarySearch(arr, key, 0, maxIndex);
  if (keyIndex !== -1) {
    return keyIndex;
  }
...
//order-agnostic binary search
auto binarySearch(arr, key, start, end) {
  //check to see if arr is sorted ascending or descending
  auto isAscending = arr[start] < arr[end];
...
### Java
```java
public static Object searchRotatedArray(arr, key) {
  start = 0;
  end = arr.length - 1;
...
### C++
```cpp
auto searchRotatedArray(arr, key) {
  auto start = 0;
  auto end = arr.size() - 1;
...
### Java
```java
public static Object searchRotatedArrayWithDuplicates(arr, key) {
  start = 0;
  end = arr.length - 1;
...
### Python
```python
def searchRotatedArrayWithDuplicates(arr, key):
    start = 0
    end = len(arr) - 1
...
### C++
```cpp
auto searchRotatedArrayWithDuplicates(arr, key) {
  auto start = 0;
  auto end = arr.size() - 1;
...
### Java
```java
public static Object countRotations(arr) {
  start = 0;
  end = arr.length - 1;
...
### Java
```java
public static Object countRotationsWithDuplicates(arr) {
  start = 0;
  end = arr.length - 1;
...
### Python
```python
def countRotationsWithDuplicates(arr):
    start = 0
    end = len(arr) - 1
...
### C++
```cpp
auto countRotationsWithDuplicates(arr) {
  auto start = 0;
  auto end = arr.size() - 1;
