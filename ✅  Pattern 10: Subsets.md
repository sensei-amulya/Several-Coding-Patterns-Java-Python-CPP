...
### Java
```java
public static Object findSubsets(nums) {
  subsets = [];
  ...
  for(i = 0; i < nums.length; i++) {
    ...
    for(j = 0; j < n; j++) {
      ...
      // set1 = subsets[j].slice(0)
      
      // set1.add(currentNumber)
      subsets.add([...subsets[j], nums[i]])
    }
  }
...
auto findSubsets(nums) {
  ...
  for(auto i = 0; i < nums.size(); i++) {
    ...
    for(auto j = 0; j < n; j++) {
      ...
      //clone the permutation
      // auto set1 = subsets[j].slice(0)
      
      // set1.push_back(currentNumber)
      subsets.push_back([...subsets[j], nums[i]])
    }
  }
...
### Java
```java
public static Object subsetsWithDupe(nums) {
...
### Java
```java
public static Object findPermutations(nums) {
  result = [];
  permutations = [[]]
  numsLength = nums.length
   ...
   for(i = 0; i < nums.length; i++) {
     ...
     for(p = 0; p < n; p++){
       ...
       System.out.println(oldPermutation)
       
       //create a new permutation by adding the current number at every position
       for(j  = 0; j < oldPermutation.length + 1; j++) {
...
auto findPermutations(nums) {
  ...
  auto numsLength = nums.size()
   ...
   for(auto i = 0; i < nums.size(); i++) {
     ...
     //we will take all existing permutations an add the
     //current number to create a new permutation
     auto n = permutations.size()
     
     for(auto p = 0; p < n; p++){
       auto oldPermutation = permutations.shift()
       std::cout << oldPermutation)
       
       //create a new permutation by adding the current number at every position
       for(auto j  = 0; j < oldPermutation.size() + 1; j++) {
         
         //clone the permutation
         auto newPermutation = oldPermutation.slice(0)
...
### Java
```java
public static Object permute(nums) {
  //recursion
  subsets = []
...
public static Object generatePermuationsRecursive(nums, index, currentPermuation, subsets) {
    if(index === nums.length) {
      subsets.add(currentPermuation)
    } else {
      //create a new permuation by adding the current number at every position
      for(i = 0; i < currentPermuation.length +1; i++) {
        newPermutation = currentPermuation.slice(0)
...
def generatePermuationsRecursive(nums, index, currentPermuation, subsets):
    if index === len(nums):
        subsets.append(currentPermuation)
        else:
            #create a new permuation by adding the current number at every position
...
auto generatePermuationsRecursive(nums, index, currentPermuation, subsets) {
    if(index === nums.size()) {
      subsets.push_back(currentPermuation)
    } else {
      //create a new permuation by adding the current number at every position
      for(auto i = 0; i < currentPermuation.size() +1; i++) {
        auto newPermutation = currentPermuation.slice(0)
...
### Java
```java
public static Object findLetterCaseStringPermutations(str) {
  permutations = [];
...
### Python
```python
def findLetterCaseStringPermutations(str):
    permutations = []
...
### C++
```cpp
auto findLetterCaseStringPermutations(str) {
  auto permutations = [];
  ...
  for(auto i = 0; i < str.size(); i++) {
    ...
    if(isNaN(parseInt(str[i]), 10)){
      ...
      for(auto j = 0; j < n; j++) {
        ...
        permutations.push_back(chs.join(''))
      }
    }
  }
  return permutations;
...
public static Object generateValidParentheses(num) {
  result = [];
  queue = []
...
def generateValidParentheses(num):
    ...
    queue.append(new ParenthesesString('', 0, 0))
    ...
    while len(queue) > 0:
        ...
        if ps.openCount === num && ps.closeCount === num:
            result.append(ps.str)
                ...
                if ps.openCount < num:
                    #if we can add an open parentheses, add it
                    queue.append(new ParenthesesString(`${ps.str}(`, ps.openCount + 1, ps.closeCount))
                if ps.openCount > ps.closeCount:
                    #if we can add a close parentheses, add it
                    queue.append(new ParenthesesString(`${ps.str})`, ps.openCount, ps.closeCount + 1))
        return result
...
auto generateValidParentheses(num) {
  auto result = [];
  auto queue = []
  
  queue.push_back(new ParenthesesString('', 0, 0))
  ...
  while(queue.size() > 0) {
    ...
    if(ps.openCount === num && ps.closeCount === num) {
      ...
      if(ps.openCount < num) {
        //if we can add an open parentheses, add it
        queue.push_back(new ParenthesesString(`${ps.str}(`, ps.openCount + 1, ps.closeCount))  
      }
      if(ps.openCount > ps.closeCount) {
        //if we can add a close parentheses, add it
        queue.push_back(new ParenthesesString(`${ps.str})`, ps.openCount, ps.closeCount + 1))
      }
    }
  }
  return result;
...
public static Object generateValidParentheses(num) {
  result = [];
  parenthesesString = Array(2 * num);
  generateValidParenthesesRecursive(num, 0, 0, parenthesesString, 0, result);
  return result;
}


public static Object generateValidParenthesesRecursive(num, openCount, closeCount, parenthesesString, index, result) {
  // if we've reached the maximum number of open and close parentheses, add to the result
  if (openCount === num && closeCount === num) {
    result.add(parenthesesString.join(''));
  } else {
...
def generateValidParenthesesRecursive(num, openCount, closeCount, parenthesesString, index, result):
    # if we've reached the maximum number of open and close parentheses, add to the result
    if openCount === num && closeCount === num:
        result.append(parenthesesString.join(''))
        else:
            if openCount < num: # if we can add an open parentheses, add it
            parenthesesString[index] = '('
            generateValidParenthesesRecursive(num, openCount + 1, closeCount, parenthesesString, index + 1, result)
...
auto generateValidParentheses(num) {
  auto result = [];
  auto parenthesesString = Array(2 * num);
  generateValidParenthesesRecursive(num, 0, 0, parenthesesString, 0, result);
  return result;
}


auto generateValidParenthesesRecursive(num, openCount, closeCount, parenthesesString, index, result) {
  // if we've reached the maximum number of open and close parentheses, add to the result
  if (openCount === num && closeCount === num) {
    result.push_back(parenthesesString.join(''));
  } else {
    if (openCount < num) { // if we can add an open parentheses, add it
      parenthesesString[index] = '(';
     generateValidParenthesesRecursive(num, openCount + 1, closeCount, parenthesesString, index + 1, result);
...
public static Object generateGeneralizedAbbreviation(word) {
  wLength = word.length
  result = [];
  queue = [];
  ...
  while(queue.length > 0) {
    ...
    if(abWord.start === wLength){
      ...
      result.add(abWord.str)
    } else {
      //continue abbreviating by incrementing the current abbreviation count
      queue.add(new AbbreviatedWord(abWord.str, abWord.start + 1, abWord.count + 1))
      
      //restart abbreviating, append the count and the current character to the string
      if(abWord.count !== 0){
        abWord.str += abWord.count
      }
      newWord = abWord.str + word[abWord.start]
      queue.add(new AbbreviatedWord(newWord, abWord.start + 1, 0))
    }
  }
  return result;
};
...
def generateGeneralizedAbbreviation(word):
    ...
    result = []
    queue = []
    queue.append(new AbbreviatedWord('', 0, 0))
    while len(queue) > 0:
        abWord = queue.shift()
        if abWord.start === wLength:
            if abWord.count !== 0:
                abWord.str += abWord.count
            result.append(abWord.str)
            else:
                #continue abbreviating by incrementing the current abbreviation count
                queue.append(new AbbreviatedWord(abWord.str, abWord.start + 1, abWord.count + 1))

                #restart abbreviating, append the count and the current character to the string
                if abWord.count !== 0:
                    abWord.str += abWord.count
                newWord = abWord.str + word[abWord.start]
                queue.append(new AbbreviatedWord(newWord, abWord.start + 1, 0))
        return result
...
auto generateGeneralizedAbbreviation(word) {
  auto wLength = word.size()
  auto result = [];
  auto queue = [];
  queue.push_back(new AbbreviatedWord('', 0, 0))
  while(queue.size() > 0) {
    auto abWord = queue.shift()
    if(abWord.start === wLength){
      if(abWord.count !== 0) {
        abWord.str += abWord.count
      }
      ...
      //continue abbreviating by incrementing the current abbreviation count
      queue.push_back(new AbbreviatedWord(abWord.str, abWord.start + 1, abWord.count + 1))
      
      //restart abbreviating, append the count and the current character to the string
      if(abWord.count !== 0){
        abWord.str += abWord.count
      }
      auto newWord = abWord.str + word[abWord.start]
      queue.push_back(new AbbreviatedWord(newWord, abWord.start + 1, 0))
    }
  }
  return result;
};
...
### Java
```java
public static Object generate_generalized_abbreviation(word) {
  result = [];
...
public static Object generate_abbreviation_recursive(word, abWord, start, count, result) {
  if (start === word.length) {
    ...
    newWord = abWord + word[start];
    generate_abbreviation_recursive(word, newWord, start + 1, 0, result);
  }
}


System.out.println(`Generalized abbreviation are: ${generate_generalized_abbreviation('BAT')}`);
System.out.println(`Generalized abbreviation are: ${generate_generalized_abbreviation('code')}`);
```
...
def generate_abbreviation_recursive(word, abWord, start, count, result):
    ...
    print(`Generalized abbreviation are: ${generate_generalized_abbreviation('BAT')}`)
    print(`Generalized abbreviation are: ${generate_generalized_abbreviation('code')}`)
```

### C++
```cpp
auto generate_generalized_abbreviation(word) {
  auto result = [];
  generate_abbreviation_recursive(word, '', 0, 0, result);
  return result;
}
...
auto generate_abbreviation_recursive(word, abWord, start, count, result) {
  if (start === word.size()) {
    if (count !== 0) {
      abWord += count;
    }
    ...
    auto newWord = abWord + word[start];
    generate_abbreviation_recursive(word, newWord, start + 1, 0, result);
  }
}


std::cout << `Generalized abbreviation are: ${generate_generalized_abbreviation('BAT')}`);
std::cout << `Generalized abbreviation are: ${generate_generalized_abbreviation('code')}`);
```
...
### Java
```java
public static Object diffWaysToEvaluateExpression(input) {
  result = [];
  
  // base case: if the input string is a number, parse and add it to output.
  if(!(input.includes('+')) && !(input.includes('-')) && !(input.includes('*'))) {
     result.add(parseInt(input))
     } else {
       for(i = 0; i < input.length; i++){
        ...
        // break the equation here into two parts and make recursively calls
           leftParts = diffWaysToEvaluateExpression(input.substring(0, i))
           rightParts = diffWaysToEvaluateExpression(input.substring(i + 1))
         
           for (l = 0; l < leftParts.length; l++) {
          for (r = 0; r < rightParts.length; r++) {
            part1 = leftParts[l],
              part2 = rightParts[r];
            if (char === '+') {
...
System.out.println(`Expression evaluations: ${diffWaysToEvaluateExpression("1+2*3")}`)//[7, 9],1+(2*3) => 7 and (1+2)*3 => 9
System.out.println(`Expression evaluations: ${diffWaysToEvaluateExpression("2*3-4-5")}`)//[8, -12, 7, -7, -3 ], 2*(3-(4-5)) => 8, 2*(3-4-5) => -12, 2*3-(4-5) => 7, 2*(3-4)-5 => -7, (2*3)-4-5 => -3
```

### Python
```python
def diffWaysToEvaluateExpression(input):
    result = []
        ...
        result.append(parseInt(input))
            ...
            for i in range(len(input)):
                ...
                if isNaN(parseInt(char, 10:)):
                    ...
                    for l in range(len(leftParts)):
                        for r in range(len(rightParts)):
                            part1 = leftParts[l],
                            part2 = rightParts[r]
                            if char === '+':
            ...
            print(`Expression evaluations: ${diffWaysToEvaluateExpression("1+2*3")}`)#[7, 9],1+(2*3) => 7 and (1+2)*3 => 9
            print(`Expression evaluations: ${diffWaysToEvaluateExpression("2*3-4-5")}`)#[8, -12, 7, -7, -3 ], 2*(3-(4-5)) => 8, 2*(3-4-5) => -12, 2*3-(4-5) => 7, 2*(3-4)-5 => -7, (2*3)-4-5 => -3
```

### C++
```cpp
auto diffWaysToEvaluateExpression(input) {
  auto result = [];
  
  // base case: if the input string is a number, parse and add it to output.
  if(!(input.includes('+')) && !(input.includes('-')) && !(input.includes('*'))) {
     result.push_back(parseInt(input))
     } else {
       for(auto i = 0; i < input.size(); i++){
        ...
        // if not a digit
        // break the equation here into two parts and make recursively calls
           auto leftParts = diffWaysToEvaluateExpression(input.substring(0, i))
           auto rightParts = diffWaysToEvaluateExpression(input.substring(i + 1))
         
           for (auto l = 0; l < leftParts.size(); l++) {
          for (auto r = 0; r < rightParts.size(); r++) {
            auto part1 = leftParts[l],
              part2 = rightParts[r];
            if (char === '+') {
...
std::cout << `Expression evaluations: ${diffWaysToEvaluateExpression("1+2*3")}`)//[7, 9],1+(2*3) => 7 and (1+2)*3 => 9
std::cout << `Expression evaluations: ${diffWaysToEvaluateExpression("2*3-4-5")}`)//[8, -12, 7, -7, -3 ], 2*(3-(4-5)) => 8, 2*(3-4-5) => -12, 2*3-(4-5) => 7, 2*(3-4)-5 => -7, (2*3)-4-5 => -3
```
...
public static Object diffWaysToEvaluateExpressionRecursive(map, input) {
     ...
     result.add(parseInt(input))
       ...
       for(i = 0; i < input.length; i++){
        ...
        // break the equation here into two parts and make recursively calls
           leftParts = diffWaysToEvaluateExpression(input.substring(0, i))
           rightParts = diffWaysToEvaluateExpression(input.substring(i + 1))
         // System.out.println(leftParts.length)
           for (l = 0; l < leftParts.length; l++) {
          for (r = 0; r < rightParts.length; r++) {
            part1 = leftParts[l],
              part2 = rightParts[r];
...
def diffWaysToEvaluateExpressionRecursive(map, input):
...
auto diffWaysToEvaluateExpressionRecursive(map, input) {
...
public static Object findUniqueTrees(n) {
   if (n <= 0) {
    return [];
  }
...
public static Object findUniqueTreesRecursive(start, end) {
  result = [];
  // treeCount = 0
  ...
  for (i = start; i < end + 1; i++) {
    ...
    for (p = 0; p < leftSubtrees.length; p++) {
      for (q = 0; q < rightSubtrees.length; q++) {
        root = new TreeNode(i, leftSubtrees[p], rightSubtrees[q]);
        result.add(root);
        // treeCount++
      }
    }
...
def findUniqueTreesRecursive(start, end):
    ...
    for i in range(start, end + 1):
        ...
        leftSubtrees = findUniqueTreesRecursive(start, i - 1)
        rightSubtrees = findUniqueTreesRecursive(i + 1, end)
        for p in range(len(leftSubtrees)):
            for q in range(len(rightSubtrees)):
                root = new TreeNode(i, leftSubtrees[p], rightSubtrees[q])
                result.append(root)
                # treeCount++
...
```cpp
class TreeNode {
  constructor(val, left = nullptr, right = nullptr) {
    this.val = val;
    this.left = left;
    this.right = right;
  }
...
auto findUniqueTreesRecursive(start, end) {
  auto result = [];
  // auto treeCount = 0
  // base condition, return 'nullptr' for an empty sub-tree
  // consider n = 1, in this case we will have start = end = 1, this means we should have only one tree
  // we will have two recursive calls, findUniqueTreesRecursive(1, 0) & (2, 1)
  // both of these should return 'nullptr' for the left and the right child
  if (start > end) {
  ...
  for (auto i = start; i < end + 1; i++) {
    // making 'i' the root of the tree
    auto leftSubtrees = findUniqueTreesRecursive(start, i - 1);
    auto rightSubtrees = findUniqueTreesRecursive(i + 1, end);
    for (auto p = 0; p < leftSubtrees.size(); p++) {
      for (auto q = 0; q < rightSubtrees.size(); q++) {
        auto root = new TreeNode(i, leftSubtrees[p], rightSubtrees[q]);
...
public static Object countTrees(n) {
if (n <= 1) {
    return 1;
  }
  count = 0;
...
if (n <= 1) {
  ...
  for (auto i = 1; i < n + 1; i++) {
    // making 'i' the root of the tree
    auto countOfLeftSubtrees = countTrees(i - 1);
    auto countOfRightSubtrees = countTrees(n - i);
    count += (countOfLeftSubtrees * countOfRightSubtrees);
  }
  return count;
};
...
public static Object countTrees(map, n) {
