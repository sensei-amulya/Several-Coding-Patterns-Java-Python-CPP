...
public static Object mergeLists(lists) {
  //push to list
...
def mergeLists(lists):
    #push to list
    if !lists || !len(lists):return None
    mergeArr = []
    resultList = new ListNode(-1)
...
auto mergeLists(lists) {
  //push to list
  if (!lists || !lists.size()) return nullptr;
  auto mergeArr = [];
  auto resultList = new ListNode(-1);
  ...
  auto curr = resultList;

  //sort list
  mergeArr
    .sort((a, b) => a - b)
    .forEach((n) => {
      auto temp = new ListNode(n);
      curr.next = temp;
      curr = curr.next;
    });
...
auto output = "Here are the elements form the merged list: ";
...
public static Object mergeLists(lists) {
  ...
  public static Object findMinNode(lists) {
    index = -1;
...
### Java
```java
public static Object findKthSmallest(lists, k) {
  minHeap = new Heap();
...
### C++
```cpp
auto findKthSmallest(lists, k) {
  auto minHeap = new Heap();
...
### Java
```java
public static Object findKthsmallest(matrix, k) {
  n = matrix.length;
  start = matrix[0][0];
  end = matrix[n - 1][n - 1];
...
public static Object countLessEqual(matrix, mid, smaller, larger) {
  count = 0;
  n = matrix.length;
  row = n - 1;
  col = 0;
...
System.out.println(`Kth smallest number is: ${findKthsmallest([[-5]], 1)}`);
...
def findKthsmallest(matrix, k):
    ...
    def countLessEqual(matrix, mid, smaller, larger):
        ...
        print(`Kth smallest number is: ${findKthsmallest([[-5]], 1)}`)
...
### C++
```cpp
auto findKthsmallest(matrix, k) {
  auto n = matrix.size();
  auto start = matrix[0][0];
  auto end = matrix[n - 1][n - 1];

  while (start < end) {
    auto mid = Math.floor(start + (end - start) / 2);

    auto [count, smaller, larger] = countLessEqual(
      matrix,
      mid,
      matrix[0][0],
      matrix[n - 1][n - 1]
...
auto countLessEqual(matrix, mid, smaller, larger) {
  auto count = 0;
  auto n = matrix.size();
  auto row = n - 1;
  auto col = 0;
...
std::cout << `Kth smallest number is: ${findKthsmallest([[-5]], 1)}`);
...
public static Object smallestRange(nums) {
  minHeap = new MinHeap();
  pointers = Array(nums.length).fill(0);
  rangeStart = 0;
  ...
  while (true) {
    ...
    minHeap.add([nums[group][pointers[group]], group]);
...
def smallestRange(nums):
    minHeap = new MinHeap()
    pointers = Array(len(nums)).fill(0)
    rangeStart = 0
    rangeEnd = float("inf")
    maxNumber = -float("inf")
    ...
    while True:
        ...
        # insert the next element into the heap
        minHeap.append([nums[group][pointers[group]], group])
        maxNumber = max(maxNumber, nums[group][pointers[group]])
...
class MinHeap {
  ...
  bubbleUp(index) {
    auto list = this.list;
    auto size = this.size;
    auto parent = Math.floor((index - 1) / 2);
...
auto smallestRange(nums) {
  auto minHeap = new MinHeap();
  auto pointers = Array(nums.size()).fill(0);
  auto rangeStart = 0;
  auto rangeEnd = INT_MAX;
  auto maxNumber = -INT_MAX;

  // put the first element of each array into the heap
  for (auto i = 0; i < nums.size(); i++) {
    minHeap.push_back([nums[i][0], i]);
    maxNumber = std::max(maxNumber, nums[i][0]);
  }

  // take the smallest(top) element from the heap, if it gives us smaller range, update the ranges
  // if the array of the top element has more elements, insert the next element in the heap
  while (true) {
    auto [minNumber, group] = minHeap.pop();
    ...
    pointers[group]++;
    if (pointers[group] >= nums[group].size()) break;

    // insert the next element into the heap
    minHeap.push_back([nums[group][pointers[group]], group]);
    maxNumber = std::max(maxNumber, nums[group][pointers[group]]);
  }

  return [rangeStart, rangeEnd];
}

std::cout << `Smallest range is: 
${smallestRange([
  [4, 10, 15, 24, 26],
  [0, 9, 12, 20],
  [5, 18, 22, 30],
...
class Heap {
  ...
  insert(elem) {
    ...
    // System.out.println(this.values[0].index)
    // this.values.sort((a, b) => b[0] - a[0]))
  }

  extract() {
    😕
    // parentIndex = this.size - 1;
    // lastIndex = 0;
...
public static Object findKLargestPairs(nums1, nums2, k) {
  ...
  heap = new Heap();
    ...
    for (i = 0; i < Math.min(k, nums1.length); i++) {
        for (j = 0; j < Math.min(k, nums2.length); j++) {
            ...
            System.out.println(elem.index, elem.val, heap.values[heap.size-1])
            if (heap.size < k) {
                heap.insert(elem);
...
def findKLargestPairs(nums1, nums2, k):
    ...
    for i in range(min(k, len(nums1))):
        for j in range(min(k, len(nums2))):
            elem =:
                index: [nums1[i], nums2[j]],
                val: nums1[i] + nums2[j]
            print(elem.index, elem.val, heap.values[heap.size-1])
            if heap.size < k:
                heap.insert(elem)
                else:if elem.val < heap.values[heap.size-1]:
                    😕
...
class Heap {
  ...
  insert(elem) {
    ...
    auto parentIndex = Math.floor((index - 1) / 2);
    ...
    // std::cout << this.values[0].index)
    // this.values.sort((a, b) => b[0] - a[0]))
  }

  extract() {
    😕
    // auto parentIndex = this.size - 1;
    // auto lastIndex = 0;
    // [this.values[parentIndex], this.values[lastIndex]] = [
    //   this.values[lastIndex],
    //   this.values[parentIndex],
...
//     auto leftChildIndex = 2 * parentIndex + 1;
//     auto rightChildIndex = 2 * parentIndex + 2;
...
auto findKLargestPairs(nums1, nums2, k) {
  if (!nums1.size() || !nums2.size()) return [];

  auto heap = new Heap();

    for (auto i = 0; i < std::min(k, nums1.size()); i++) {
        for (auto j = 0; j < std::min(k, nums2.size()); j++) {
            auto elem = {
                index: [nums1[i], nums2[j]],
                val: nums1[i] + nums2[j]
            }
            std::cout << elem.index, elem.val, heap.values[heap.size-1])
            if (heap.size < k) {
                heap.insert(elem);
            } else if (elem.val < heap.values[heap.size-1]) {
              😕
...
std::cout << `Pairs with largest sum are:
${findKLargestPairs([9, 8, 2], [6, 3, 1], 3)}`);
//[9, 3], [9, 6], [8, 6]
// These 3 pairs have the largest sum. No other pair has a sum larger than any of these.
std::cout << 
...
public static Object findKthSmallestNumber(nums, k) {
  ...
  previousSmallestNum = -Integer.MAX_VALUE,
    previousSmallestIndex = -1;
  (currentSmallestNum = Integer.MAX_VALUE), (currentSmallestIndex = -1);
  for (i = 0; i < k; i++) {
...
def findKthSmallestNumber(nums, k):
    # to handle duplicates, we will keep track of previous smallest number and its index
    previousSmallestNum = -float("inf"),
    previousSmallestIndex = -1
    (currentSmallestNum = float("inf")), (currentSmallestIndex = -1)
    for i in range(k):
...
auto findKthSmallestNumber(nums, k) {
  // to handle duplicates, we will keep track of previous smallest number and its index
  auto previousSmallestNum = -INT_MAX,
    previousSmallestIndex = -1;
  (currentSmallestNum = INT_MAX), (currentSmallestIndex = -1);
  for (i = 0; i < k; i++) {
...
public static Object findKthSmallestNumber_rec(nums, k, start, end) {
...
public static Object partition(nums, low, high) {
  if (low === high) {
    return low;
  }
...
auto findKthSmallestNumber_rec(nums, k, start, end) {
  auto p = partition(nums, start, end);
...
def partition(nums, low, high):
    ...
    pivotIndex = int(Math.random() * (high - low + 1)) + low
    [nums[pivotIndex], nums[high]] = [nums[high], nums[pivotIndex]]
...
auto partition(nums, low, high) {
  ...
  auto pivotIndex = Math.floor(Math.random() * (high - low + 1)) + low;
  [nums[pivotIndex], nums[high]] = [nums[high], nums[pivotIndex]];
...
public static Object median_of_medians(nums, low, high) {
  n = high - low + 1;
  ...
  for (i = 0; i < nums.length; i += 5) {
    if (i + 5 <= nums.length) {
      partitions.add(nums.slice(i, i + 5));
    }
  }
...
def median_of_medians(nums, low, high):
    ...
    for (i = 0; i < len(nums); i += 5):
        if i + 5 <= len(nums):
            partitions.append(nums.slice(i, i + 5))
...
auto partition(nums, low, high) {
  ...
  auto median = median_of_medians(nums, low, high);
  // find the median in the array and swap it with 'nums[high]' which will become our pivot
  for (i = low; i < high; i++) {
    if (nums[i] === median) {
      [nums[i], nums[high]] = [nums[high], nums[i]];
...
auto median_of_medians(nums, low, high) {
  n = high - low + 1;
  // if we have less than 5 elements, ignore the partitioning algorithm
  if (n < 5) {
    return nums[low];
  ...
  // for simplicity, lets ignore any partition with less than 5 elements
  auto partitions = [];
  for (auto i = 0; i < nums.size(); i += 5) {
    if (i + 5 <= nums.size()) {
      partitions.push_back(nums.slice(i, i + 5));
    }
  }
  ...
  auto medians = [];
  partitions.forEach((p) => {
    medians.push_back(p[2]);
  });

  return partition(medians, 0, medians.size() - 1);
}
