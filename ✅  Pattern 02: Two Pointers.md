...
### Java
```java
public static Object pairWithTargetSum(nums, target) {
...
### C++
```cpp
auto pairWithTargetSum(nums, target) {
...
### Java
```java
public static Object pairWithTargetSum(arr, target) {
  start = 0
  end = arr.length-1
...
### C++
```cpp
auto pairWithTargetSum(arr, target) {
  auto start = 0
  auto end = arr.size()-1
  
  while(start < end) {
    auto currentSum = arr[start] + arr[end]
...
### Java
```java
public static Object removeDuplicates(arr) {
...
### Java
```java
public static Object removeElement(arr, key) {
...
### Java
```java
public static Object makeSquares(arr) {
...
auto makeSquares(arr) {
  ...
  auto n = arr.size();
  auto squares = Array(n).fill(0)
  auto highestSquareIndex = n - 1
  auto start = 0
  auto end = n-1
  while(start<= end) {
    auto startSquare = arr[start] * arr[start]
    auto endSquare = arr[end] * arr[end]
...
### Java
```java
public static Object searchTriplets(arr) {
  arr.sort((a, b) => a - b);
  triplets = [];
...
public static Object searchPair(arr, targetSum, start, triplets) {
  end = arr.length - 1;
  ...
  while (start < end) {
    ...
    if (currentSum === targetSum) {
      //found the triplet
      triplets.add([-targetSum, arr[start], arr[end]]);
      start++;
      end--;
...
def searchPair(arr, targetSum, start, triplets):
    end = len(arr) - 1
    ...
    while start < end:
        ...
        if currentSum === targetSum:
            #found the triplet
            triplets.append([-targetSum, arr[start], arr[end]])
            start++
            end--
            while start < end && arr[start] === arr[start - 1]:
                #skip same element to avoid duplicates
...
auto searchPair(arr, targetSum, start, triplets) {
  auto end = arr.size() - 1;
...
### Java
```java
public static Object tripletSumCloseToTarget(arr, targetSum){
  arr.sort((a, b) => a - b)
  ...
  for(i = 0; i < arr.length - 2; i++) {
    ...
    while(start < end) {
      targetDifference = targetSum - arr[i] - arr[start] - arr[end]
...
### C++
```cpp
auto tripletSumCloseToTarget(arr, targetSum){
  arr.sort((a, b) => a - b)
  
  auto smallestDifference = INT_MAX
  ...
  for(auto i = 0; i < arr.size() - 2; i++) {
    ...
    while(start < end) {
      auto targetDifference = targetSum - arr[i] - arr[start] - arr[end]
      ...
      if(std::abs(targetDifference) < std::abs(smallestDifference)) {
        //save the closet difference
        smallestDifference = targetDifference
      }
      //the second part of the followinf 'if' is to handle the smallest sum
      //when we have more than one solution
      if(std::abs(targetDifference) < std::abs(smallestDifference) || (std::abs(targetDifference) === std::abs(smallestDifference) && targetDifference > smallestDifference)) {
        //save the closest and the biggest difference
        smallestDifference = targetDifference
      }
...
### Java
```java
public static Object tripletWithSmallerSum (arr, target) {
  arr.sort((a, b) => a -b)
  count = 0;
...
public static Object searchPair(arr, targetSum, first){
  count = 0
  start = first + 1
  end = arr.length -1
...
### C++
```cpp
auto tripletWithSmallerSum (arr, target) {
  arr.sort((a, b) => a -b)
  auto count = 0;
...
auto searchPair(arr, targetSum, first){
  auto count = 0
  auto start = first + 1
  auto end = arr.size() -1
...
public static Object searchPair(arr, targetSum, first, triplets){
  ...
  while(start < end) {
    if(arr[start] + arr[end] < targetSum) {
      ...
      //by any number between start and end to get a sum less than the targetSum
      for(i = end; i > start; i--){
        triplets.add(arr[first], arr[start], arr[end])
      }
      start++
    } else {
      //we need a pair with a smaller sum
...
def searchPair(arr, targetSum, first, triplets):
    ...
    while start < end:
        if arr[start] + arr[end] < targetSum:
            ...
            #by any number between start and end to get a sum less than the targetSum
            for(i = end; i > start; i--):
                triplets.append(arr[first], arr[start], arr[end])
            start++
            else:
                #we need a pair with a smaller sum
                end--
...
auto searchPair(arr, targetSum, first, triplets){
  ...
  while(start < end) {
    if(arr[start] + arr[end] < targetSum) {
      ...
      for(auto i = end; i > start; i--){
        triplets.push_back(arr[first], arr[start], arr[end])
      }
      start++
    } else {
      //we need a pair with a smaller sum
...
### Java
```java
public static Object findSubarrays(arr, target) {
  result = []
  product = 1
  start = 0
...
def findSubarrays(arr, target):
    ...
    for end in range(len(arr)):
        product *= arr[end]
        while product >= target && start < len(arr):
            product /= arr[start]
            start++
        #since the product of all numbers from start to end is less than the target.
        #Therefore, all subarrays from start to end will have a product less than the target too;
...
auto findSubarrays(arr, target) {
  ...
  for(auto end = 0; end < arr.size(); end++) {
    product *= arr[end]
    while(product >= target && start < arr.size()) {
      product /= arr[start]
      start++
    }
    //since the product of all numbers from start to end is less than the target. 
...
### Java
```java
public static Object dutchFlagSort(arr) {
...
System.out.println(dutchFlagSort([1, 0, 2, 1, 0]))//[0 0 1 1 2]
System.out.println(dutchFlagSort([2, 2, 0, 1, 2, 0]))//[0 0 1 2 2 2 ]
```
...
def dutchFlagSort(arr):
    ...
    while i <= high:
        ...
        print(dutchFlagSort([1, 0, 2, 1, 0]))#[0 0 1 1 2]
        print(dutchFlagSort([2, 2, 0, 1, 2, 0]))#[0 0 1 2 2 2 ]
```

### C++
```cpp
auto dutchFlagSort(arr) {
  //all elements < low are 0, and all elements > high are 2
  //all elements >= low < i are 1
  auto low = 0
...
std::cout << dutchFlagSort([1, 0, 2, 1, 0]))//[0 0 1 1 2]
std::cout << dutchFlagSort([2, 2, 0, 1, 2, 0]))//[0 0 1 2 2 2 ]
```
...
### Java
```java
public static Object searchQuads (arr, target) {
  //sort the array
  arr.sort((a,b) => a -b)
...
public static Object searchPairs(arr, targetSum, first, second, quads) {
  start = second + 1
  end = arr.length -1
  
  while(start < end) {
    sum = arr[first] + arr[second] + arr[start] + arr[end]
    if(sum === targetSum) {
      //found the quadruplet
      quads.add([arr[first], arr[second], arr[start], arr[end]])
      start++
      end--
      while(start < end && arr[start] === arr[start -1]){
        //skip the same element to avoid duplicate quadruplets
...
def searchPairs(arr, targetSum, first, second, quads):
    start = second + 1
    end = len(arr) -1
    ...
    while start < end:
        ...
        if sum === targetSum:
            #found the quadruplet
            quads.append([arr[first], arr[second], arr[start], arr[end]])
            start++
            end--
            while start < end && arr[start] === arr[start -1]:
                #skip the same element to avoid duplicate quadruplets
...
auto searchPairs(arr, targetSum, first, second, quads) {
  auto start = second + 1
  auto end = arr.size() -1
  
  while(start < end) {
    auto sum = arr[first] + arr[second] + arr[start] + arr[end]
    if(sum === targetSum) {
      //found the quadruplet
...
### Java
```java
public static Object backspaceCompare(str1, str2) {
  //use two pointer approach to compare the strings
  pointerOne = str1.length -1
  pointerTwo = str2.length -1
  
  while(pointerOne >= 0 || pointerTwo >= 0){
    i = getNextChar(str1, pointerOne)
    j = getNextChar(str2, pointerTwo)
...
public static Object getNextChar(str, index) {
  backspaceCount = 0
  while(index >= 0) {
    if(str[index] === '#'){
...
def backspaceCompare(str1, str2):
    #use two pointer approach to compare the strings
    pointerOne = len(str1) -1
    pointerTwo = len(str2) -1
...
### C++
```cpp
auto backspaceCompare(str1, str2) {
  //use two pointer approach to compare the strings
  auto pointerOne = str1.size() -1
  auto pointerTwo = str2.size() -1
  
  while(pointerOne >= 0 || pointerTwo >= 0){
    auto i = getNextChar(str1, pointerOne)
    auto j = getNextChar(str2, pointerTwo)
...
auto getNextChar(str, index) {
  auto backspaceCount = 0
  while(index >= 0) {
    if(str[index] === '#'){
      //found a backspace character
      backspaceCount++
...
### Java
```java
public static Object shortestWindowSort(arr) {
  low = 0
  high = arr.length - 1
  ...
  subArrayMax = -Integer.MAX_VALUE
  subArrayMin = Integer.MAX_VALUE
...
def shortestWindowSort(arr):
    ...
    #find the first number out of sorting order from the beginning
    while low < len(arr) -1 && arr[low] <= arr[low+1]:
        low++
    if low === len(arr) - 1:
        # if the array is already sorted
        return 0
    ...
    while low > 0 && arr[low -1] > subArrayMin:
        low--
    #extend the subarray to include any number which is small than the maximum of the subarray
    while high < len(arr) - 1 && arr[high + 1] < subArrayMax:
        high++
...
auto shortestWindowSort(arr) {
  ...
  //find the max and min of the subarray
  auto subArrayMax = -INT_MAX
  auto subArrayMin = INT_MAX
  
  for(auto k = low; k < high + 1; k++) {
    subArrayMax = std::max(subArrayMax, arr[k])
    subArrayMin = std::min(subArrayMin, arr[k])
  }
