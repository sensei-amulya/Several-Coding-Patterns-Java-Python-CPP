...
public static Object hasPath(root, sum) {
  if (!root) {
    return false;
  }
...
def hasPath(root, sum):
    ...
    #if the current node is a leaf and it's value is
    #equal to the sum then we've found a path
    if root.value === sum && root.left === None && root.right === None:
        return True
...
root.right.left = new TreeNode(10)
root.right.right = new TreeNode(5)
print(`Tree has path: ${hasPath(root, 23)}`)
print(`Tree has path: ${hasPath(root, 16)}`)
```
...
root.right.left = new TreeNode(10);
root.right.right = new TreeNode(5);
std::cout << `Tree has path: ${hasPath(root, 23)}`);
std::cout << `Tree has path: ${hasPath(root, 16)}`);
```
...
public static Object findPaths(root, sum) {
  allPaths = []
...
public static Object findAPath(currentNode, sum, currentPath, allPaths) {
  if(currentNode === null) {
    return
  }
...
def findAPath(currentNode, sum, currentPath, allPaths):
    if currentNode === None:
        return
    ...
    #if the current node is a leaf and it's value is
    #equal to sum, save the current path
    if currentNode.value === sum && currentNode.left === None && currentNode.right === None:
...
auto findAPath(currentNode, sum, currentPath, allPaths) {
  ...
  //if the current node is a leaf and it's value is 
  //equal to sum, save the current path
  if(currentNode.value === sum && currentNode.left === nullptr && currentNode.right === nullptr) {
...
public static Object findPaths(root) {
  allPaths = []
...
public static Object findAPath(currentNode, currentPath, allPaths) {
  if(currentNode === null) {
    return
  }
...
def findAPath(currentNode, currentPath, allPaths):
    if currentNode === None:
        return
    ...
    #if the current node is not a leaf, save the current path
    if currentNode.left === None && currentNode.right === None:
        allPaths.append([...currentPath])
        else:
            #traverse the left sub-tree
            findAPath(currentNode.left, currentPath, allPaths)
...
auto findAPath(currentNode, currentPath, allPaths) {
  if(currentNode === nullptr) {
    return
  }
...
public static Object maxPathSum(root) {
  maxSum = -Integer.MAX_VALUE
  
  public static Object findAPath(currentNode, currentPath) {
    if(currentNode === null) {
      return
    }
    //add the current node to the path
...
def maxPathSum(root):
    ...
    def findAPath(currentNode, currentPath):
        if currentNode === None:
            return
        #add the current node to the path
        currentPath.append(currentNode.value)
...
auto maxPathSum(root) {
  auto maxSum = -INT_MAX
  
  auto findAPath(currentNode, currentPath) {
    if(currentNode === nullptr) {
      return
    }
    //add the current node to the path
    ...
    //if the current node is not a leaf, save the current path
    if(currentNode.left === nullptr && currentNode.right === nullptr) {
      for(auto i = 0; i < currentPath.size(); i++) {
        pathMax += currentPath[i] 
      }
      maxSum = std::max(maxSum, pathMax)
    } else {
      //traverse the left sub-tree
      findAPath(currentNode.left, currentPath)
      //traverse the right sub-tree
...
public static Object findSumOfPathNumbers(root) {
  return findRootToLeafPathNumbers(root, 0)
}

public static Object findRootToLeafPathNumbers(currentNode, pathSum) {
    if(currentNode === null) {
      return 0
    } 
...
root.right.left = new TreeNode(6)
root.right.right = new TreeNode(5)
print(`Total Sum of Path Numbers: ${findSumOfPathNumbers(root)}`)
```
...
auto findSumOfPathNumbers(root) {
  return findRootToLeafPathNumbers(root, 0)
}

auto findRootToLeafPathNumbers(currentNode, pathSum) {
    if(currentNode === nullptr) {
      return 0
    } 
...
root.right.left = new TreeNode(6)
root.right.right = new TreeNode(5)
std::cout << `Total Sum of Path Numbers: ${findSumOfPathNumbers(root)}`)
```
- The time complexity of the above algorithm is `O(N)`, where `N` is the total number of nodes in the tree. This is due to the fact that we traverse each node once.
- The space complexity of the above algorithm will be `O(N)` in the worst case. This space will be used to store the <b>recursion</b> stack. The worst case will happen when the given tree is a <i>linked list</i> (i.e., every node has only one child).
...
public static Object findPath(root, sequence) {
  if(!root) {
...
public static Object findPathRecursive(currentNode, sequence, sequenceIndex) {
...
System.out.println(`Tree has path sequence: ${findPath(root, [1, 0, 7])}`)
System.out.println(`Tree has path sequence: ${findPath(root, [1, 1, 6])}`)
```
...
def findPathRecursive(currentNode, sequence, sequenceIndex):
    ...
    #if the current node is a leaf, and it is the end of the sequence, then we have found a path
    if currentNode.left === None && currentNode.right === None && sequenceIndex === sequenceLength - 1:return True
...
print(`Tree has path sequence: ${findPath(root, [1, 0, 7])}`)
print(`Tree has path sequence: ${findPath(root, [1, 1, 6])}`)
```
...
auto findPathRecursive(currentNode, sequence, sequenceIndex) {
  if(currentNode === nullptr) return false
  
  auto sequenceLength = sequence.size()
  if(sequenceIndex >= sequenceLength || currentNode.value !== sequence[sequenceIndex]) return false
  
  //if the current node is a leaf, and it is the end of the sequence, then we have found a path
  if(currentNode.left === nullptr && currentNode.right === nullptr && sequenceIndex === sequenceLength - 1) return true
...
std::cout << `Tree has path sequence: ${findPath(root, [1, 0, 7])}`)
std::cout << `Tree has path sequence: ${findPath(root, [1, 1, 6])}`)
```
- The time complexity of the above algorithm is `O(N)`, where `N` is the total number of nodes in the tree. This is due to the fact that we traverse each node once.
- The space complexity of the above algorithm will be `O(N)` in the worst case. This space will be used to store the <b>recursion</b> stack. The worst case will happen when the given tree is a <i>linked list</i> (i.e., every node has only one child).
...
public static Object countPaths(root, S) {
  currentPath = []
  return countPathsRecursive(root, S, currentPath)
}

public static Object countPathsRecursive(currentNode, S, currentPath) {
...
```python
class TreeNode:
    constructor(value, right = None, left = None):
        this.value = value
        this.right = right
        this.left = left
...
root.right.left = new TreeNode(10)
root.right.right = new TreeNode(5)
print(`Tree has ${countPaths(root, 11)} paths`)
```

### C++
```cpp
class TreeNode {
  constructor(value, right = nullptr, left = nullptr) {
    this.value = value
    this.right = right
    this.left = left
  }
...
auto countPaths(root, S) {
  auto currentPath = []
  return countPathsRecursive(root, S, currentPath)
}

auto countPathsRecursive(currentNode, S, currentPath) {
  ...
  //find the sums of all aub-paths in the current path list
  for(auto i = currentPath.size() - 1; i >= 0; i--) {
    pathSum += currentPath[i]
...
root.right.left = new TreeNode(10)
root.right.right = new TreeNode(5)
std::cout << `Tree has ${countPaths(root, 11)} paths`)
```
  ...
  findDiameter(root) {
   ...
   calculateHeight(currentNode) {
    if(currentNode === null) return 0
     ...
     leftTreeHeight = this.calculateHeight(currentNode.left)
     rightTreeHeight = this.calculateHeight(currentNode.right)
...
class TreeDiameter:
    ...
    calculateHeight(currentNode):
        ...
        if leftTreeHeight !== 0 && rightTreeHeight !== 0:
            ...
            #update the global tree diameter
            this.treeDiameter = max(this.treeDiameter, diameter)
  ...
  findDiameter(root) {
   ...
   calculateHeight(currentNode) {
    if(currentNode === nullptr) return 0
     ...
     auto leftTreeHeight = this.calculateHeight(currentNode.left)
     auto rightTreeHeight = this.calculateHeight(currentNode.right)
     ...
     if(leftTreeHeight !== 0 && rightTreeHeight !== 0) {
       //diameter at the currentNode will be equal to the height of the left 
       //sub-tree the height of sub-trees + 1 for the currentNode
       auto diameter = leftTreeHeight + rightTreeHeight + 1
       
       //update the global tree diameter
       this.treeDiameter = std::max(this.treeDiameter, diameter)
     }
...
auto treeDiameter = new TreeDiameter()
auto root = new TreeNode(1)
root.left = new TreeNode(2)
root.right = new TreeNode(3)
root.left.left = new TreeNode(4)
root.right.left = new TreeNode(5)
root.right.right = new TreeNode(6)
std::cout << `Tree Diameter: ${treeDiameter.findDiameter(root)}`)
...
public static Object findMaximumPathSum(root) {
  globalMaximumSum = -Integer.MAX_VALUE
  findMaximumPathSumRecursive(root)
  
  
  public static Object findMaximumPathSumRecursive(currentNode) {
    if(currentNode === null) return 0
    
    maxPathSumLeft = findMaximumPathSumRecursive(currentNode.left)
    maxPathSumRight = findMaximumPathSumRecursive(currentNode.right)
    ...
    localMaximumSum = maxPathSumLeft + maxPathSumRight + currentNode.value
...
def findMaximumPathSum(root):
    globalMaximumSum = -float("inf")
    findMaximumPathSumRecursive(root)


    def findMaximumPathSumRecursive(currentNode):
        if currentNode === None:return 0
...
root.left = new TreeNode(2)
root.right = new TreeNode(3)
print(`Maximum Path Sum: ${findMaximumPathSum(root)}`)#6
...
root.right.left.right = new TreeNode(8)
root.right.right.left = new TreeNode(9)
print(`Maximum Path Sum: ${findMaximumPathSum(root)}`)#31

root = new TreeNode(-1)
root.left = new TreeNode(-3)
print(`Maximum Path Sum: ${findMaximumPathSum(root)}`)
```
...
auto findMaximumPathSum(root) {
  auto globalMaximumSum = -INT_MAX
  findMaximumPathSumRecursive(root)
  
  
  auto findMaximumPathSumRecursive(currentNode) {
    if(currentNode === nullptr) return 0
    
    auto maxPathSumLeft = findMaximumPathSumRecursive(currentNode.left)
    auto maxPathSumRight = findMaximumPathSumRecursive(currentNode.right)
    
    //ignore paths with negative sums, since we need to find the maximum sum 
    //we should ignore any path which has an overall negative sum
    maxPathSumLeft = std::max(maxPathSumLeft, 0)
    maxPathSumRight = std::max(maxPathSumRight, 0)
    
    //maximum path sum at the currentNode will be equal to the sum from the
    //left subtree + the sum from the right subtree + value of the currentNode
    auto localMaximumSum = maxPathSumLeft + maxPathSumRight + currentNode.value
    
    //update the globalMaximumSum 
    globalMaximumSum = std::max(globalMaximumSum, localMaximumSum)
    
    //maximum sum of any path from the currentNode will be equal to the maximum
    //of the sums from left to right sub-tree plus the value of the currentNode
    return std::max(maxPathSumLeft, maxPathSumRight) + currentNode.value
  }
  return globalMaximumSum
};
...
root.left = new TreeNode(2)
root.right = new TreeNode(3)
std::cout << `Maximum Path Sum: ${findMaximumPathSum(root)}`)//6
...
root.right.left.right = new TreeNode(8)
root.right.right.left = new TreeNode(9)
std::cout << `Maximum Path Sum: ${findMaximumPathSum(root)}`)//31

root = new TreeNode(-1)
root.left = new TreeNode(-3)
std::cout << `Maximum Path Sum: ${findMaximumPathSum(root)}`)
```
