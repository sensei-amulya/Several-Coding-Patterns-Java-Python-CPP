...
### Java
```java
public static Object calculateFibonacci(n) {
  if (n < 2) return n;
...
print(`5th Fibonacci is ---> ${calculateFibonacci(5)}`)
print(`6th Fibonacci is ---> ${calculateFibonacci(6)}`)
print(`7th Fibonacci is ---> ${calculateFibonacci(7)}`)
```

### C++
```cpp
auto calculateFibonacci(n) {
  if (n < 2) return n;

  return calculateFibonacci(n - 1) + calculateFibonacci(n - 2);
}

std::cout << `5th Fibonacci is ---> ${calculateFibonacci(5)}`);
std::cout << `6th Fibonacci is ---> ${calculateFibonacci(6)}`);
std::cout << `7th Fibonacci is ---> ${calculateFibonacci(7)}`);
```
...
### Java
```java
public static Object solveKnapsack(profits, weights, capacity) {
  public static Object knapsackRecursive(profits, wights, capacity, currIndex) {
    //check base case
...
def solveKnapsack(profits, weights, capacity):
    def knapsackRecursive(profits, wights, capacity, currIndex):
        #check base case
        if capacity <= 0 || currIndex >= len(profits):return 0
...
### C++
```cpp
auto solveKnapsack(profits, weights, capacity) {
  auto knapsackRecursive(profits, wights, capacity, currIndex) {
    //check base case
    ...
    // recursively process the remaining capacity and items
    // WITHOUT item at currIndex
    auto currentProfitMinusIndexItem = knapsackRecursive(
      profits,
      weights,
      capacity,
      currIndex + 1
    ...
    // return the set from the above two sets with higher profit
    return std::max(currentProfit, currentProfitMinusIndexItem);
  }
...
public static Object solveKnapsack(profits, weights, capacity) {
  ...
  public static Object knapsackRecursive(profits, weights, capacity, currIndex) {
    //check base case
    if (capacity <= 0 || currIndex >= profits.length) return 0;
...
def solveKnapsack(profits, weights, capacity):
    ...
    def knapsackRecursive(profits, weights, capacity, currIndex):
        ...
        if typeof memo[currIndex][capacity] !== 'None':
            return memo[currIndex][capacity]
...
auto solveKnapsack(profits, weights, capacity) {
  ...
  auto knapsackRecursive(profits, weights, capacity, currIndex) {
    //check base case
    if (capacity <= 0 || currIndex >= profits.size()) return 0;
    ...
    // return the set from the above two sets with higher profit
    memo[currIndex][capacity] = std::max(
      currentProfit,
      currentProfitMinusIndexItem
    );
    return memo[currIndex][capacity];
...
def solveKnapsack(profits, weights, capacity):
    ...
    if capacity <= 0 || n == 0 || len(weights) != n:return 0
...
auto solveKnapsack(profits, weights, capacity) {
  ...
  for (auto i = 1; i < n; i++) {
    for (auto c = 1; c <= capacity; c++) {
      ...
      //take the maximum
      dp[i][c] = std::max(profitWithI, profitMinusI);
      // std::cout << dp)
    }
  }
  //maximum profit with be at the bottom-right corner
...
public static Object solveKnapsack(profits, weights, capacity) {
  ...
  //**public static Object to print the set of items included in the knapsack**//
  selectedWeights = '';
  totalProfit = dp[weights.length - 1][capacity];
...
auto solveKnapsack(profits, weights, capacity) {
  ...
  //**auto to print the set of items included in the knapsack**//
  auto selectedWeights = '';
  auto totalProfit = dp[weights.size() - 1][capacity];
  auto remainingCapacity = capacity;
...
public static Object solveKnapsack(profits, weights, capacity) {
  ...
  //**public static Object to print the set of items included in the knapsack**
  selectedWeights = '';
  totalProfit = dp[(weights.length - 1) % 2][capacity];
  remainingCapacity = capacity;
  for (i = weights.length - 1; i > 0; i--) {
...
def solveKnapsack(profits, weights, capacity):
    ...
    for i in range(1, n):
        ...
        #**function to print the set of items included in the knapsack**
        selectedWeights = ''
        totalProfit = dp[(len(weights) - 1) % 2][capacity]
        remainingCapacity = capacity
        for (i = len(weights) - 1; i > 0; i--):
            if totalProfit != dp[(i - 1:% 2][remainingCapacity]):
                selectedWeights = `{${weights[i]}lbs @ $${profits[i]}}${selectedWeights}`
...
auto solveKnapsack(profits, weights, capacity) {
  ...
  for (auto i = 1; i < n; i++) {
    for (auto c = 1; c <= capacity; c++) {
      ...
      //take the maximum
      dp[i % 2][c] = std::max(profitWithI, profitMinusI);
    }
  }
  //**auto to print the set of items included in the knapsack**
  auto selectedWeights = '';
  auto totalProfit = dp[(weights.size() - 1) % 2][capacity];
  auto remainingCapacity = capacity;
  for (auto i = weights.size() - 1; i > 0; i--) {
    if (totalProfit != dp[(i - 1) % 2][remainingCapacity]) {
      selectedWeights = `{${weights[i]}lbs @ $${profits[i]}}${selectedWeights}`;
...
auto solveKnapsack(profits, weights, capacity) {
  ...
  //the only difference is that
  //we use i%2 instead of i and (i-1)%2 instead of i-1
  auto dp = Array(capacity + 1).fill(0);
  ...
  for (auto i = 1; i < n; i++) {
    for (auto c = capacity; c >= 0; c--) {
      ...
      //take the maximum
      dp[c] = std::max(profitWithI, profitMinusI);
    }
  }
  //**auto to print the set of items included in the knapsack**
  auto selectedWeights = '';
  auto totalProfit = dp[capacity];
  auto remainingCapacity = capacity;
  //*look into this for loop
  // for (auto i = weights.size() - 1; i > 0; i--) {
  //   if (totalProfit != dp[(i - 1) % 2][remainingCapacity]) {
...
### Java
```java
public static Object canPartition(num) {
  //brute force
  sum = 0;
  ...
  public static Object canPartitionRecursive(num, sum, currIndex) {
...
System.out.println(`Can partition: ${canPartition([1, 2, 3, 4])}`); //True
//The given set can be partitioned into two subsets with equal sum: {1, 4} & {2, 3}
System.out.println(`Can partition: ${canPartition([1, 1, 3, 4, 7])}`); //True
//The given set can be partitioned into two subsets with equal sum: {1, 3, 4} & {1, 7}
System.out.println(`Can partition: ${canPartition([2, 3, 4, 6])}`); //False
...
def canPartition(num):
    ...
    for i in range(len(num)):
        ...
        def canPartitionRecursive(num, sum, currIndex):
            #recursive base case check
            if sum === 0:return True

            if len(num) === 0 || currIndex >= len(num):return False
    ...
    print(`Can partition: ${canPartition([1, 2, 3, 4])}`); #True
    #The given set can be partitioned into two subsets with equal sum: {1, 4} & {2, 3}
    print(`Can partition: ${canPartition([1, 1, 3, 4, 7])}`); #True
    #The given set can be partitioned into two subsets with equal sum: {1, 3, 4} & {1, 7}
    print(`Can partition: ${canPartition([2, 3, 4, 6])}`); #False
    #The given set cannot be partitioned into two subsets with equal sum.
```

### C++
```cpp
auto canPartition(num) {
  //brute force
  auto sum = 0;
  for (auto i = 0; i < num.size(); i++) sum += num[i];
  ...
  auto canPartitionRecursive(num, sum, currIndex) {
    //recursive base case check
    if (sum === 0) return true;
...
std::cout << `Can partition: ${canPartition([1, 2, 3, 4])}`); //True
//The given set can be partitioned into two subsets with equal sum: {1, 4} & {2, 3}
std::cout << `Can partition: ${canPartition([1, 1, 3, 4, 7])}`); //True
//The given set can be partitioned into two subsets with equal sum: {1, 3, 4} & {1, 7}
std::cout << `Can partition: ${canPartition([2, 3, 4, 6])}`); //False
//The given set cannot be partitioned into two subsets with equal sum.
```
...
public static Object canPartition(num) {
  ...
  public static Object canPartitionRecursive(dp, num, sum, currIndex) {
    //recursive base case check
    if (sum === 0) return true;
...
def canPartition(num):
    ...
    for i in range(len(num)):
        ...
        def canPartitionRecursive(dp, num, sum, currIndex):
            #recursive base case check
            if sum === 0:return True
            ...
            #if we have not already processed a similar problem
            if typeof dp[currIndex][sum] === 'None':
                #recursive call after choosing the number at currIndex
                #if the number at currIndex exceed the sum, we shouldn't process
                if num[currIndex] <= sum:
                    if canPartitionRecursive(dp, num, sum - num[currIndex], currIndex + 1:)
...
auto canPartition(num) {
  ...
  auto canPartitionRecursive(dp, num, sum, currIndex) {
    //recursive base case check
    if (sum === 0) return true;
...
### Java
```java
public static Object canPartition(nums, sum) {
  //bottom-up dynamic programming approach
  n = nums.length;
  ...
  for (s = 1; s <= sum; s++) dp[0][s] = nums[0] === s;
...
System.out.println(`Can partitioning be done: ---> ${canPartition([1, 2, 3, 4], 6)}`);
//True
...
System.out.println(`Can partitioning be done: ---> ${canPartition([1, 3, 4, 8], 6)}`);
//False
...
def canPartition(nums, sum):
    ...
    for i in range(n):
        ...
        for s in range(1, = sum):
            ...
            print(`Can partitioning be done: ---> ${canPartition([1, 2, 3, 4], 6)}`)
            #True
            #The given set has a subset whose sum is '6': {1, 2, 3}
            ...
            print(`Can partitioning be done: ---> ${canPartition([1, 3, 4, 8], 6)}`)
            #False
            #The given set does not have any subset whose sum is equal to '6'.
```
...
auto canPartition(nums, sum) {
  ...
  //with only one number, we can form a subset only when the required sum is equal to its value
  for (auto s = 1; s <= sum; s++) dp[0][s] = nums[0] === s;
...
std::cout << `Can partitioning be done: ---> ${canPartition([1, 2, 3, 4], 6)}`);
//True
//The given set has a subset whose sum is '6': {1, 2, 3}
...
std::cout << `Can partitioning be done: ---> ${canPartition([1, 3, 4, 8], 6)}`);
//False
//The given set does not have any subset whose sum is equal to '6'.
```
...
auto canPartition(nums, sum) {
  ...
  auto dp = Array(sum + 1).fill(false);

  //sum=0, as we can always have 0 sum with an empty set
  dp[0] = true;

  //with only one number, we can form a subset only when the required sum is equal to its value
  for (auto s = 1; s <= sum; s++) dp[s] = nums[0] == s;

  //process all subsets for all lsum
  for (auto i = 1; i < nums.size(); i++) {
    for (auto s = sum; s >= 0; s--) {
      // if dp[s]==true, this means we can get the sum s without
      //num[i], then move on to the next number else we can include num[i]
      //and see if e can find a subset to get the remaining sum
...
### Java
```java
public static Object canPartition(nums) {
  //brute force

  public static Object canPartitionRecursive(nums, currIndex, sum1, sum2) {
    ...
    difference1 = canPartitionRecursive(
      nums,
      currIndex + 1,
      sum1 + nums[currIndex],
      sum2
    ...
    difference2 = canPartitionRecursive(
      nums,
      currIndex + 1,
      sum1,
      sum2 + nums[currIndex]
...
System.out.println(`Can partitioning be done: ---> ${canPartition([1, 2, 3, 9])}`);
//3
...
System.out.println(`Can partitioning be done: ---> ${canPartition([1, 2, 7, 1, 5])}`);
//0
...
System.out.println(`Can partitioning be done: ---> ${canPartition([1, 3, 100, 4])}`);
//92
...
def canPartition(nums):
    ...
    def canPartitionRecursive(nums, currIndex, sum1, sum2):
        #recursive base check
        if currIndex === len(nums):return abs(sum1 - sum2)
...
print(`Can partitioning be done: ---> ${canPartition([1, 2, 3, 9])}`)
#3
#We can partition the given set into two subsets where minimum absolute difference between the sum of numbers is '3'. Following are the two subsets: {1, 2, 3} & {9}.

print(`Can partitioning be done: ---> ${canPartition([1, 2, 7, 1, 5])}`)
#0
#We can partition the given set into two subsets where minimum absolute difference between the sum of number is '0'. Following are the two subsets: {1, 2, 5} & {7, 1}.

print(`Can partitioning be done: ---> ${canPartition([1, 3, 100, 4])}`)
#92
#We can partition the given set into two subsets where minimum absolute difference between the sum of numbers is '92'. Here are the two subsets: {1, 3, 4} & {100}.
```
...
### C++
```cpp
auto canPartition(nums) {
  //brute force

  auto canPartitionRecursive(nums, currIndex, sum1, sum2) {
    //recursive base check
    if (currIndex === nums.size()) return std::abs(sum1 - sum2);

    //recursive call after including the number at the
    //currIndex in the first set
    auto difference1 = canPartitionRecursive(
      nums,
      currIndex + 1,
      sum1 + nums[currIndex],
      sum2
    ...
    //recursive call after including the number at the
    //currIndex in the second set
    auto difference2 = canPartitionRecursive(
      nums,
      currIndex + 1,
      sum1,
      sum2 + nums[currIndex]
    ...
    return std::min(difference1, difference2);
  }
  return canPartitionRecursive(nums, 0, 0, 0);
}

std::cout << `Can partitioning be done: ---> ${canPartition([1, 2, 3, 9])}`);
//3
//We can partition the given set into two subsets where minimum absolute difference between the sum of numbers is '3'. Following are the two subsets: {1, 2, 3} & {9}.

std::cout << `Can partitioning be done: ---> ${canPartition([1, 2, 7, 1, 5])}`);
//0
//We can partition the given set into two subsets where minimum absolute difference between the sum of number is '0'. Following are the two subsets: {1, 2, 5} & {7, 1}.

std::cout << `Can partitioning be done: ---> ${canPartition([1, 3, 100, 4])}`);
//92
//We can partition the given set into two subsets where minimum absolute difference between the sum of numbers is '92'. Here are the two subsets: {1, 3, 4} & {100}.
```
...
def canPartition(nums):
    ...
    for i in range(len(nums)):
        ...
        def canPartitionRecursive(nums, currIndex, sum1, sum2):
            ...
            #check if we have not already process similar problem
            if typeof dp[currIndex][sum1] === 'None':
                #recursive call after including the number at the
                #currIndex in the first set
                difference1 = canPartitionRecursive(
                nums,
...
auto canPartition(nums) {
  ...
  auto canPartitionRecursive(nums, currIndex, sum1, sum2) {
    ...
    if (typeof dp[currIndex][sum1] === 'undefined') {
      ...
      dp[currIndex][sum1] = std::min(difference1, difference2);
    }
    return dp[currIndex][sum1];
  }
...
auto canPartition(nums) {
  ...
  auto requiredSum = Math.floor(sum / 2);
  auto dp = Array(n)
    .fill(false)
    .map(() => Array(requiredSum + 1).fill(false));
...
### Java
```java
public static Object countSubsets(num, sum) {
  public static Object countSubsetsRecursive(num, sum, currIndex) {
    //recursive base case check
    if (sum === 0) return 1;
    ...
    //recursive call after excluding the number at currIndex
    sum2 = countSubsetsRecursive(num, sum, currIndex + 1);
    return sum1 + sum2;
  }
...
System.out.println(`Count of subset sum is: ---> ${countSubsets([1, 1, 2, 3], 4)}`);
// 3
//The given set has '3' subsets whose sum is '4': {1, 1, 2}, {1, 3}, {1, 3}
//Note that we have two similar sets {1, 3}, because we have two '1' in our input.
System.out.println(`Count of subset sum is: ---> ${countSubsets([1, 2, 7, 1, 5], 9)}`);
//3
...
### Python
```python
def countSubsets(num, sum):
    def countSubsetsRecursive(num, sum, currIndex):
        #recursive base case check
        if sum === 0:return 1

        if len(num) === 0 || currIndex >= len(num):return 0
...
print(`Count of subset sum is: ---> ${countSubsets([1, 1, 2, 3], 4)}`)
# 3
#The given set has '3' subsets whose sum is '4': {1, 1, 2}, {1, 3}, {1, 3}
#Note that we have two similar sets {1, 3}, because we have two '1' in our input.
print(`Count of subset sum is: ---> ${countSubsets([1, 2, 7, 1, 5], 9)}`)
#3
#The given set has '3' subsets whose sum is '9': {2, 7}, {1, 7, 1}, {1, 2, 1, 5}
```
...
### C++
```cpp
auto countSubsets(num, sum) {
  auto countSubsetsRecursive(num, sum, currIndex) {
    //recursive base case check
    if (sum === 0) return 1;
    ...
    //recursive call after excluding the number at currIndex
    auto sum2 = countSubsetsRecursive(num, sum, currIndex + 1);
    return sum1 + sum2;
  }
...
std::cout << `Count of subset sum is: ---> ${countSubsets([1, 1, 2, 3], 4)}`);
// 3
//The given set has '3' subsets whose sum is '4': {1, 1, 2}, {1, 3}, {1, 3}
//Note that we have two similar sets {1, 3}, because we have two '1' in our input.
std::cout << `Count of subset sum is: ---> ${countSubsets([1, 2, 7, 1, 5], 9)}`);
//3
//The given set has '3' subsets whose sum is '9': {2, 7}, {1, 7, 1}, {1, 2, 1, 5}
```
...
### Java
```java
public static Object findTargetSubsets(num, s) {
  totalSum = 0;

  for (i = 0; i < num.length; i++) totalSum += num[i];
...
public static Object countSubsets(num, sum) {
  // this public static Object is the exactly similar to what we
...
### C++
```cpp
auto findTargetSubsets(num, s) {
  auto totalSum = 0;

  for (auto i = 0; i < num.size(); i++) totalSum += num[i];
...
auto countSubsets(num, sum) {
  // this auto is the exactly similar to what we
  //have in 'Count of Subsets Sum' problem
  auto n = num.size();
...
public static Object countSubsets(num, sum) {
  // this public static Object is the exactly simialar to what we
  //have in 'Count of Subsets Sum' problem
  n = num.length;
...
auto countSubsets(num, sum) {
  // this auto is the exactly simialar to what we
  //have in 'Count of Subsets Sum' problem
  auto n = num.size();
...
def solveKnapsack(profits, weights, capacity):
    def knapsackRecursive(profits, weights, capacity, currIndex):
        ...
        capacity <= 0 ||
        len(profits) === 0 ||
        len(weights) !== len(profits) ||
        currIndex >= len(profits)
        )
        return 0
...
auto solveKnapsack(profits, weights, capacity) {
  auto knapsackRecursive(profits, weights, capacity, currIndex) {
    ...
    if (
      capacity <= 0 ||
      profits.size() === 0 ||
      weights.size() !== profits.size() ||
      currIndex >= profits.size()
    )
      return 0;
...
auto solveKnapsack(profits, weights, capacity) {
  ...
  for (auto i = 0; i < n; i++) {
    for (auto c = 1; c <= capacity; c++) {
      auto currentProfit = 0;
      auto currentProfitMinusIndex = 0;
...
### Java
```java
public static Object solveRodCutting(lengths, prices, n) {
  public static Object solveRodCuttingRecursive(lengths, prices, n, currIndex) {
    //recursive base case check
    if (
      n <= 0 ||
      prices.length === 0 ||
    ...
    //recursive call after excluding the element at the currIndex
    currentProfitMinusIndexItem = solveRodCuttingRecursive(
      prices,
      lengths,
      n - lengths[currIndex],
      currIndex + 1
...
def solveRodCutting(lengths, prices, n):
    def solveRodCuttingRecursive(lengths, prices, n, currIndex):
        #recursive base case check
        if (
        n <= 0 ||
        len(prices) === 0 ||
        len(lengths) !== len(prices) ||
        currIndex >= len(prices)
        )
        return 0
...
### C++
```cpp
auto solveRodCutting(lengths, prices, n) {
  auto solveRodCuttingRecursive(lengths, prices, n, currIndex) {
    //recursive base case check
    if (
      n <= 0 ||
      prices.size() === 0 ||
      lengths.size() !== prices.size() ||
      currIndex >= prices.size()
    )
      return 0;
    ...
    //recursive call after excluding the element at the currIndex
    auto currentProfitMinusIndexItem = solveRodCuttingRecursive(
      prices,
      lengths,
      n - lengths[currIndex],
      currIndex + 1
...
def solveRodCutting(lengths, prices, n):
    #base checks
    if n <= 0 || len(prices) === 0 || len(prices) !== len(lengths):
        return 0
...
auto solveRodCutting(lengths, prices, n) {
  ...
  auto lCount = lengths.size();
  auto dp = Array(lCount)
    .fill(0)
    .map(() => Array(n + 1).fill(0));
  ...
  for (auto i = 0; i < lCount; i++) {
    for (auto len = 1; len <= n; len++) {
      ...
      dp[i][len] = std::max(pointer1, pointer2);
    }
  }
...
### Java
```java
public static Object countChange(denominations, total) {
  public static Object countChangeRecursive(denominations, total, currIndex) {
    //base checks
    if (total === 0) return 1;
    ...
    sumAtNextIndex = countChangeRecursive(
      denominations,
      total,
      currIndex + 1
    );
...
def countChange(denominations, total):
    def countChangeRecursive(denominations, total, currIndex):
        #base checks
        if total === 0:return 1

        if len(denominations) === 0 || currIndex >= len(denominations):
            return 0
...
### C++
```cpp
auto countChange(denominations, total) {
  auto countChangeRecursive(denominations, total, currIndex) {
    //base checks
    if (total === 0) return 1;
    ...
    //recursive call after excluding the coin at the currIndex
    auto sumAtNextIndex = countChangeRecursive(
      denominations,
      total,
      currIndex + 1
    );
...
def countChange(denominations, total):
    ...
    def countChangeRecursive(denominations, total, currIndex):
        ...
        if len(denominations) === 0 || currIndex >= len(denominations):
            ...
            #if we have already processed a similar sub-problem, return the result
            if typeof dp[currIndex][total] !== 'None':
                return dp[currIndex][total]
...
public static Object countChange(denominations, total) {
  public static Object countChangeRecursive(denominations, total, currIndex) {
    ...
    currCoinCount = Integer.MAX_VALUE;
    if (denominations[currIndex] <= total) {
      nextCoinCount = countChangeRecursive(
        denominations,
        total - denominations[currIndex],
        currIndex
      );
      if (nextCoinCount !== Integer.MAX_VALUE) currCoinCount = nextCoinCount + 1;
    }
    //recursive call after excluding the coin at currIndex
    currCountMinusIndex = countChangeRecursive(
      denominations,
      total,
      currIndex + 1
    );
  ...
  result = countChangeRecursive(denominations, total, 0);
  return result === Integer.MAX_VALUE ? -1 : result;
}

System.out.println(`Number of ways to make change: ---> ${countChange([1, 2, 3], 5)}`);
System.out.println(
  `Number of ways to make change: ---> ${countChange([1, 2, 3], 11)}`
);
System.out.println(`Number of ways to make change: ---> ${countChange([1, 2, 3], 7)}`);
System.out.println(`Number of ways to make change: ---> ${countChange([3, 5], 7)}`);
```
    ...
    def countChangeRecursive(denominations, total, currIndex):
        ...
        if len(denominations) === 0 || currIndex >= len(denominations):
            ...
            if denominations[currIndex] <= total:
                ...
                if nextCoinCount !== float("inf"):currCoinCount = nextCoinCount + 1
            #recursive call after excluding the coin at currIndex
            currCountMinusIndex = countChangeRecursive(
            denominations,
            total,
        ...
        result = countChangeRecursive(denominations, total, 0)
        return result === float("inf") ? -1 : result

    print(`Number of ways to make change: ---> ${countChange([1, 2, 3], 5)}`)
    print(
    `Number of ways to make change: ---> ${countChange([1, 2, 3], 11)}`
    )
    print(`Number of ways to make change: ---> ${countChange([1, 2, 3], 7)}`)
    print(`Number of ways to make change: ---> ${countChange([3, 5], 7)}`)
```
...
auto countChange(denominations, total) {
  auto countChangeRecursive(denominations, total, currIndex) {
    ...
    //recursive call after selecting the coin at currIndex
    //if the coin at currIndex exceeds the total, we won't process
    auto currCoinCount = INT_MAX;
    if (denominations[currIndex] <= total) {
      auto nextCoinCount = countChangeRecursive(
        denominations,
        total - denominations[currIndex],
        currIndex
      );
    ...
    //recursive call after excluding the coin at currIndex
    auto currCountMinusIndex = countChangeRecursive(
      denominations,
      total,
      currIndex + 1
    );
    return std::min(currCoinCount, currCountMinusIndex);
  }

  auto result = countChangeRecursive(denominations, total, 0);
  return result === INT_MAX ? -1 : result;
}

std::cout << `Number of ways to make change: ---> ${countChange([1, 2, 3], 5)}`);
std::cout << 
  `Number of ways to make change: ---> ${countChange([1, 2, 3], 11)}`
);
std::cout << `Number of ways to make change: ---> ${countChange([1, 2, 3], 7)}`);
std::cout << `Number of ways to make change: ---> ${countChange([3, 5], 7)}`);
```
...
auto countChange(denominations, total) {
  ...
  auto countChangeRecursive(denominations, total, currIndex) {
    ...
    if (typeof dp[currIndex][total] === 'undefined') {
      ...
      dp[currIndex][total] = std::min(currCoinCount, currCountMinusIndex);
    }
...
def countChange(denominations, total):
    ...
    for i in range(n):
        ...
        print(dp)
        #total combos will be in the bottom-right corner
        return dp[n - 1][total] === float("inf") ? -1 : dp[n - 1][total]
...
auto countChange(denominations, total) {
  ...
  for (auto i = 0; i < n; i++) {
    for (auto t = 1; t <= total; t++) {
      ...
      if (t >= denominations[i]) {
        //include the coin
        dp[i][t] = std::min(dp[i][t], dp[i][t - denominations[i]] + 1);
      }
    }
  }
...
### Java
```java
public static Object countRibbonPieces(ribbonLengths, total) {
  public static Object countRibbonPiecesRecursive(ribbonLengths, total, currIndex) {
    //base check
    if (total === 0) return 0;
    ...
    currRibbon = -Integer.MAX_VALUE;
    if (ribbonLengths[currIndex] <= total) {
      withoutIndexRibbon = countRibbonPiecesRecursive(
        ribbonLengths,
        total - ribbonLengths[currIndex],
        currIndex
      );

      if (withoutIndexRibbon !== -Integer.MAX_VALUE) currRibbon = withoutIndexRibbon + 1;
    }

    //recursive call after excluding the ribbon at currIndex
    nextRibbon = countRibbonPiecesRecursive(
      ribbonLengths,
      total,
      currIndex + 1
    );
  ...
  result = countRibbonPiecesRecursive(ribbonLengths, total, 0);
  return result === -Integer.MAX_VALUE ? -1 : result;
}
...
def countRibbonPieces(ribbonLengths, total):
    def countRibbonPiecesRecursive(ribbonLengths, total, currIndex):
        #base check
        if total === 0:return 0

        if len(ribbonLengths) === 0 || currIndex >= len(ribbonLengths):
            return -float("inf")

            #recursive call after selecting the ribbon length at currIndex
            #if the ribbon length at the currIndex exceeds total, we shouldn't process
            currRibbon = -float("inf")
            if ribbonLengths[currIndex] <= total:
                withoutIndexRibbon = countRibbonPiecesRecursive(
                ribbonLengths,
                total - ribbonLengths[currIndex],
                ...
                if withoutIndexRibbon !== -float("inf"):currRibbon = withoutIndexRibbon + 1
...
### C++
```cpp
auto countRibbonPieces(ribbonLengths, total) {
  auto countRibbonPiecesRecursive(ribbonLengths, total, currIndex) {
    //base check
    if (total === 0) return 0;

    if (ribbonLengths.size() === 0 || currIndex >= ribbonLengths.size())
      return -INT_MAX;

    //recursive call after selecting the ribbon length at currIndex
    //if the ribbon length at the currIndex exceeds total, we shouldn't process
    auto currRibbon = -INT_MAX;
    if (ribbonLengths[currIndex] <= total) {
      auto withoutIndexRibbon = countRibbonPiecesRecursive(
        ribbonLengths,
        total - ribbonLengths[currIndex],
        currIndex
      );

      if (withoutIndexRibbon !== -INT_MAX) currRibbon = withoutIndexRibbon + 1;
    }

    //recursive call after excluding the ribbon at currIndex
    auto nextRibbon = countRibbonPiecesRecursive(
      ribbonLengths,
      total,
      currIndex + 1
    );
    return std::min(currRibbon, nextRibbon);
  }
  auto result = countRibbonPiecesRecursive(ribbonLengths, total, 0);
  return result === -INT_MAX ? -1 : result;
}
    ...
    for i in range(n):
        ...
        for i in range(n):
            for t in range(1, = total):
                ...
                if t >= ribbonLengths[i] && dp[i][t - ribbonLengths[i]] !== -float("inf"):
                    #include the ribbon and check if the remaining length can be cut into available lengths
...
auto countRibbonPieces(ribbonLengths, total) {
  ...
  for (auto i = 0; i < n; i++) {
    for (auto t = 1; t <= total; t++) {
      ...
      if (t >= ribbonLengths[i] && dp[i][t - ribbonLengths[i]] !== -INT_MAX) {
        //include the ribbon and check if the remaining length can be cut into available lengths
        dp[i][t] = std::max(dp[i][t], dp[i][t - ribbonLengths[i]] + 1);
      }
    }
  }
...
### Java
```java
public static Object countWays(n) {
  if (n === 0) {
    return 1;
  ...
  take1Step = countWays(n - 1);
  // similarly, if we took 2 steps, we are left with 'n-2' steps;
  take2Step = countWays(n - 2);
  // if we took 3 steps, we are left with 'n-3' steps;
  take3Step = countWays(n - 3);
...
def countWays(n):
    if n === 0:
        ...
        if n === 1:
            ...
            print(`Number of ways: ---> ${countWays(3)}`)
            print(`Number of ways: ---> ${countWays(4)}`)
            print(`Number of ways: ---> ${countWays(5)}`)
```
...
auto countWays(n) {
  ...
  // if we take 1 step, we are left with 'n-1' steps;
  auto take1Step = countWays(n - 1);
  // similarly, if we took 2 steps, we are left with 'n-2' steps;
  auto take2Step = countWays(n - 2);
  // if we took 3 steps, we are left with 'n-3' steps;
  auto take3Step = countWays(n - 3);

  return take1Step + take2Step + take3Step;
}

std::cout << `Number of ways: ---> ${countWays(3)}`);
std::cout << `Number of ways: ---> ${countWays(4)}`);
std::cout << `Number of ways: ---> ${countWays(5)}`);
```
...
public static Object countWays(n) {
  ...
  public static Object countWaysRecursive(n) {
    // base case
    if (n <= 2) {
      return dp[n];
    }
    ...
    // if we take 1 step, we are left with 'n-1' steps;
    take1Step = countWaysRecursive(n - 1);
    // similarly, if we took 2 steps, we are left with 'n-2' steps;
    take2Step = countWaysRecursive(n - 2);
    // if we took 3 steps, we are left with 'n-3' steps;
    take3Step = countWaysRecursive(n - 3);
...
System.out.println(`Number of ways: ---> ${countWays(0)}`);
System.out.println(`Number of ways: ---> ${countWays(1)}`);
System.out.println(`Number of ways: ---> ${countWays(2)}`);
System.out.println(`Number of ways: ---> ${countWays(3)}`);
System.out.println(`Number of ways: ---> ${countWays(4)}`);
System.out.println(`Number of ways: ---> ${countWays(5)}`);
```
...
print(`Number of ways: ---> ${countWays(0)}`)
print(`Number of ways: ---> ${countWays(1)}`)
print(`Number of ways: ---> ${countWays(2)}`)
print(`Number of ways: ---> ${countWays(3)}`)
print(`Number of ways: ---> ${countWays(4)}`)
print(`Number of ways: ---> ${countWays(5)}`)
```
...
auto countWays(n) {
  ...
  auto countWaysRecursive(n) {
    ...
    // if we take 1 step, we are left with 'n-1' steps;
    auto take1Step = countWaysRecursive(n - 1);
    // similarly, if we took 2 steps, we are left with 'n-2' steps;
    auto take2Step = countWaysRecursive(n - 2);
    // if we took 3 steps, we are left with 'n-3' steps;
    auto take3Step = countWaysRecursive(n - 3);
    dp[n] = take1Step + take2Step + take3Step;
...
std::cout << `Number of ways: ---> ${countWays(0)}`);
std::cout << `Number of ways: ---> ${countWays(1)}`);
std::cout << `Number of ways: ---> ${countWays(2)}`);
std::cout << `Number of ways: ---> ${countWays(3)}`);
std::cout << `Number of ways: ---> ${countWays(4)}`);
std::cout << `Number of ways: ---> ${countWays(5)}`);
```
...
```cpp
auto countWays(n) {
  auto dp = Array(n + 1).fill(1);
  dp[2] = 2;

  for (auto i = 3; i <= n; i++) {
    // if we take 1 step, we are left with 'n-1' steps;
    auto take1Step = dp[n - 1];
    // similarly, if we took 2 steps, we are left with 'n-2' steps;
    auto take2Step = dp[n - 2];
    // if we took 3 steps, we are left with 'n-3' steps;
    auto take3Step = dp[n - 3];
...
System.out.println(`Number of ways: ---> ${countWays(4)}`);
System.out.println(`Number of ways: ---> ${countWays(5)}`);
System.out.println(`Number of ways: ---> ${countWays(6)}`);
```
...
print(`Number of ways: ---> ${countWays(4)}`)
print(`Number of ways: ---> ${countWays(5)}`)
print(`Number of ways: ---> ${countWays(6)}`)
```
...
auto countWays(n) {
  auto countWaysRecursive(n) {
    ...
    // if we subtract 1, we are left with 'n-1'
    auto subtract1 = countWays(n - 1);
    // if we subtract 3, we are left with 'n-3'
    auto subtract3 = countWays(n - 3);
    // if we subtract 4, we are left with 'n-4'
    auto subtract4 = countWays(n - 4);
...
std::cout << `Number of ways: ---> ${countWays(4)}`);
std::cout << `Number of ways: ---> ${countWays(5)}`);
std::cout << `Number of ways: ---> ${countWays(6)}`);
```
...
auto countWays(n) {
  ...
  auto countWaysRecursive(n) {
    ...
    // if(typeod dp[n] === 'undefined'){
    // if we subtract 1, we are left with 'n-1'
    auto subtract1 = countWaysRecursive(n - 1);
    // if we subtract 3, we are left with 'n-3'
    auto subtract3 = countWaysRecursive(n - 3);
    // if we subtract 4, we are left with 'n-4'
    auto subtract4 = countWaysRecursive(n - 4);
...
### Java
```java
public static Object countMinJumps(jumps) {
  public static Object countMinJumpsRecursive(jumps, jumpIndex) {
...
System.out.println(`Minimum jumps needed: ---> ${countMinJumps([2, 1, 1, 1, 4])}`);
System.out.println(
...
### Python
```python
def countMinJumps(jumps):
    def countMinJumpsRecursive(jumps, jumpIndex):
        #if we have reached the last index
        #we don't need to do any more jumping
        if jumpIndex >= len(jumps) - 1:return 0

        if jumps[jumpIndex] === 0:return float("inf")

        totalJumps = float("inf")
        start = jumpIndex + 1
        end = jumpIndex + jumps[jumpIndex]
        while start < len(jumps) && start <= end:
            #jump one step and recurse for the remainder
            minJumps = countMinJumpsRecursive(jumps, start++)
...
print(`Minimum jumps needed: ---> ${countMinJumps([2, 1, 1, 1, 4])}`)
print(
`Minimum jumps needed: ---> ${countMinJumps([1, 1, 3, 6, 9, 3, 0, 1, 3])}`
)
```
...
### C++
```cpp
auto countMinJumps(jumps) {
  auto countMinJumpsRecursive(jumps, jumpIndex) {
    //if we have reached the last index
    //we don't need to do any more jumping
    ...
    auto totalJumps = INT_MAX;
    auto start = jumpIndex + 1;
    auto end = jumpIndex + jumps[jumpIndex];
    while (start < jumps.size() && start <= end) {
      //jump one step and recurse for the remainder
      auto minJumps = countMinJumpsRecursive(jumps, start++);

      if (minJumps !== INT_MAX) {
        totalJumps = std::min(totalJumps, minJumps + 1);
      }
    }
...
std::cout << `Minimum jumps needed: ---> ${countMinJumps([2, 1, 1, 1, 4])}`);
std::cout << 
  `Minimum jumps needed: ---> ${countMinJumps([1, 1, 3, 6, 9, 3, 0, 1, 3])}`
);
```
...
```cpp
auto countMinJumps(jumps) {
  auto dp = Array(jumps.size()).fill(0);
  auto countMinJumpsRecursive(jumps, jumpIndex) {
    //if we have reached the last index
    //we don't need to do any more jumping
    if (jumpIndex >= jumps.size() - 1) return 0;
...
```java
public static Object countMinJumps(jumps) {
  dp = Array(jumps.length).fill(Integer.MAX_VALUE);
  dp[0] = 0;

  for (start = 0; start < jumps.length - 1; start++) {
    for (
      end = start + 1;
...
```python
def countMinJumps(jumps):
    dp = Array(len(jumps)).fill(float("inf"))
    dp[0] = 0

    for start in range(len(jumps) - 1):
        for (
        end = start + 1
        end <= start + jumps[start] && end < len(jumps)
        end++
        ):
...
```cpp
auto countMinJumps(jumps) {
  auto dp = Array(jumps.size()).fill(INT_MAX);
  dp[0] = 0;

  for (auto start = 0; start < jumps.size() - 1; start++) {
    for (
      auto end = start + 1;
      end <= start + jumps[start] && end < jumps.size();
      end++
    ) {
...
### Java
```java
public static Object findMinFee(fee) {
  public static Object findMinFeeRecursive(fee, currIndex) {
    if (currIndex > fee.length - 1) return 0;

    //if we take 1 step, we are left with n-1 steps
    take1Step = findMinFeeRecursive(fee, currIndex + 1);
    //similarly, if we take 2 steps, we are left with n-2 steps
    take2Steps = findMinFeeRecursive(fee, currIndex + 2);
    //if we take 3 steps, we are left with n-3 steps
    take3Steps = findMinFeeRecursive(fee, currIndex + 3);

    minCost = Math.min(take1Step, take2Steps, take3Steps);
...
System.out.println(`Minimum fee needed: ---> ${findMinFee([1, 2, 5, 2, 1, 2])}`);
System.out.println(`Minimum fee needed: ---> ${findMinFee([2, 3, 4, 5])}`);
```

### Python
```python
def findMinFee(fee):
    def findMinFeeRecursive(fee, currIndex):
        if currIndex > len(fee) - 1:return 0
        ...
        minCost = min(take1Step, take2Steps, take3Steps)

        return minCost + fee[currIndex]
    return findMinFeeRecursive(fee, 0)

print(`Minimum fee needed: ---> ${findMinFee([1, 2, 5, 2, 1, 2])}`)
print(`Minimum fee needed: ---> ${findMinFee([2, 3, 4, 5])}`)
```

### C++
```cpp
auto findMinFee(fee) {
  auto findMinFeeRecursive(fee, currIndex) {
    if (currIndex > fee.size() - 1) return 0;

    //if we take 1 step, we are left with n-1 steps
    auto take1Step = findMinFeeRecursive(fee, currIndex + 1);
    //similarly, if we take 2 steps, we are left with n-2 steps
    auto take2Steps = findMinFeeRecursive(fee, currIndex + 2);
    //if we take 3 steps, we are left with n-3 steps
    auto take3Steps = findMinFeeRecursive(fee, currIndex + 3);

    auto minCost = std::min(take1Step, take2Steps, take3Steps);
...
std::cout << `Minimum fee needed: ---> ${findMinFee([1, 2, 5, 2, 1, 2])}`);
std::cout << `Minimum fee needed: ---> ${findMinFee([2, 3, 4, 5])}`);
```
...
def findMinFee(fee):
    ...
    def findMinFeeRecursive(fee, currIndex):
        ...
        dp[currIndex] =
        min(take1Step, take2Steps, take3Steps) + fee[currIndex]
...
auto findMinFee(fee) {
  ...
  auto findMinFeeRecursive(fee, currIndex) {
    ...
    dp[currIndex] =
      std::min(take1Step, take2Steps, take3Steps) + fee[currIndex];
...
```cpp
auto findMinFee(fee) {
  auto dp = Array(fee.size() + 1).fill(0);
  // if there are no steps, we dont have to pay any fee
  // only one step, so we have to pay its fee
  dp[1] = fee[0];
  // for 2 steps, since we start from the first step, so we have to pay its fee
...
### Java
```java
public static Object findMaxSteal(wealth) {
  public static Object findMaxStealRecursive(wealth, currIndex) {
    ...
    skipCurr = findMaxStealRecursive(wealth, currIndex + 1);
...
System.out.println(`Maximum stealing: ---> ${findMaxSteal([2, 5, 1, 3, 6, 2, 4])}`);
System.out.println(`Maximum stealing: ---> ${findMaxSteal([2, 10, 14, 8, 1])}`);
```

### Python
```python
def findMaxSteal(wealth):
    def findMaxStealRecursive(wealth, currIndex):
        if currIndex >= len(wealth):return 0
...
print(`Maximum stealing: ---> ${findMaxSteal([2, 5, 1, 3, 6, 2, 4])}`)
print(`Maximum stealing: ---> ${findMaxSteal([2, 10, 14, 8, 1])}`)
```

### C++
```cpp
auto findMaxSteal(wealth) {
  auto findMaxStealRecursive(wealth, currIndex) {
    if (currIndex >= wealth.size()) return 0;
    ...
    //skip current house to steal from the adjacent house
    auto skipCurr = findMaxStealRecursive(wealth, currIndex + 1);

    return std::max(stealCurr, skipCurr);
  }
  return findMaxStealRecursive(wealth, 0);
}

std::cout << `Maximum stealing: ---> ${findMaxSteal([2, 5, 1, 3, 6, 2, 4])}`);
std::cout << `Maximum stealing: ---> ${findMaxSteal([2, 10, 14, 8, 1])}`);
```
...
auto findMaxSteal(wealth) {
  ...
  auto findMaxStealRecursive(wealth, currIndex) {
    ...
    dp[currIndex] = std::max(stealCurr, skipCurr);
    return dp[currIndex];
  }
  return findMaxStealRecursive(wealth, 0);
}
...
public static Object findMaxSteal(wealth) {
  ...
  dp = Array(wealth.length + 1).fill(0);
...
auto findMaxSteal(wealth) {
  //+1 to handle the 0 index house
  auto dp = Array(wealth.size() + 1).fill(0);
  //if there are no houses, the thief can't steal anything
  //only one house, so the thief will only be able to steal from that single house
  dp[1] = wealth[1];

  //please note that dp[] has one extra to handle house zero
  for (auto i = 1; i < wealth.size(); i++) {
    dp[i + 1] = std::max(wealth[i] + dp[i - 1], dp[i]);
  }
...
```python
def findMaxSteal(wealth):
    if len(wealth) === 0:return 0

    house1 = 0
    house2 = wealth[0]
    for i in range(1, len(wealth)):
        [house1, house2] = [house2, max(house1 + wealth[i], house2)]
...
auto findMaxSteal(wealth) {
  ...
  auto house1 = 0;
  auto house2 = wealth[0];
  for (auto i = 1; i < wealth.size(); i++) {
    [house1, house2] = [house2, std::max(house1 + wealth[i], house2)];
  }
...
### Java
```java
public static Object findLPSLength(str) {
  public static Object findLPSLengthRecursive(str, startIndex, endIndex) {
    //base checks
    ...
    startSkip = findLPSLengthRecursive(str, startIndex + 1, endIndex);
    endSkip = findLPSLengthRecursive(str, startIndex, endIndex - 1);
...
### Python
```python
def findLPSLength(str):
    def findLPSLengthRecursive(str, startIndex, endIndex):
        #base checks
        if startIndex > endIndex:return 0
    ...
    return findLPSLengthRecursive(str, 0, len(str) - 1)
...
### C++
```cpp
auto findLPSLength(str) {
  auto findLPSLengthRecursive(str, startIndex, endIndex) {
    //base checks
    if (startIndex > endIndex) return 0;
    ...
    //case 2: skip one element either from the start or end
    auto startSkip = findLPSLengthRecursive(str, startIndex + 1, endIndex);
    auto endSkip = findLPSLengthRecursive(str, startIndex, endIndex - 1);

    return std::max(startSkip, endSkip);
  }
  return findLPSLengthRecursive(str, 0, str.size() - 1);
}
    ...
    def findLPSLengthRecursive(str, startIndex, endIndex):
        ...
        if str[startIndex] === str[endIndex]:
            ...
            2 + findLPSLengthRecursive(str, startIndex + 1, endIndex - 1)
                ...
                dp[startIndex][endIndex] = max(startSkip, endSkip)
...
auto findLPSLength(str) {
  ...
  auto findLPSLengthRecursive(str, startIndex, endIndex) {
    ...
    if (str[startIndex] === str[endIndex]) {
      ...
      dp[startIndex][endIndex] = std::max(startSkip, endSkip);
    }
...
dp[startIndex][endIndex] = max(dp[startIndex + 1][endIndex], dp[startInde[endIndex - 1])
```
...
if st[endIndex] == st[startIndex]
  ...
  dp[startIndex][endIndex] = std::max(dp[startIndex + 1][endIndex], dp[startInde[endIndex - 1])
```
...
public static Object findLPSLength(str) {
  ...
  for (startIndex = str.length - 1; startIndex >= 0; startIndex--) {
...
System.out.println('Length of LPS ---> ' + findLPSLength('abdbca'));
// Output: 5
// Explanation: LPS is "abdba".

System.out.println('Length of LPS ---> ' + findLPSLength('cddpd'));
// Output: 3
// Explanation: LPS is "ddd".

System.out.println('Length of LPS ---> ' + findLPSLength('pqr'));
// Output: 1
// Explanation: LPS could be "p", "q" or "r".
```
...
def findLPSLength(str):
    ...
    dp = Array(len(str))
    .fill(0)
    .map(() => Array(len(str)).fill(0))

    #every sequence with 1 element is a palindrome with length of 1
    for start in range(len(str)):
        dp[start][start] = 1

    for (startIndex = len(str) - 1; startIndex >= 0; startIndex--):
        for endIndex in range(startIndex + 1, len(str)):
            #case 1: elements ar the start and end are the same
            if str.charAt(startIndex:=== str.charAt(endIndex)):
                dp[startIndex][endIndex] = 2 + dp[startIndex + 1][endIndex - 1]
                else:
    ...
    print('Length of LPS ---> ' + findLPSLength('abdbca'))
    # Output: 5
    # Explanation: LPS is "abdba".

    print('Length of LPS ---> ' + findLPSLength('cddpd'))
    # Output: 3
    # Explanation: LPS is "ddd".

    print('Length of LPS ---> ' + findLPSLength('pqr'))
    # Output: 1
    # Explanation: LPS could be "p", "q" or "r".
```
...
auto findLPSLength(str) {
  //dp[i][j] stores the length of LPS from index start to end
  auto dp = Array(str.size())
    .fill(0)
    .map(() => Array(str.size()).fill(0));

  //every sequence with 1 element is a palindrome with length of 1
  for (auto start = 0; start < str.size(); start++) {
    dp[start][start] = 1;
  }

  for (auto startIndex = str.size() - 1; startIndex >= 0; startIndex--) {
    for (auto endIndex = startIndex + 1; endIndex < str.size(); endIndex++) {
      //case 1: elements ar the start and end are the same
      if (str.charAt(startIndex) === str.charAt(endIndex)) {
        dp[startIndex][endIndex] = 2 + dp[startIndex + 1][endIndex - 1];
      } else {
        //case 2: skip one element either from the start or end
        dp[startIndex][endIndex] = std::max(
          dp[startIndex + 1][endIndex],
          dp[startIndex][endIndex - 1]
        );
      }
...
std::cout << 'Length of LPS ---> ' + findLPSLength('abdbca'));
// Output: 5
// Explanation: LPS is "abdba".

std::cout << 'Length of LPS ---> ' + findLPSLength('cddpd'));
// Output: 3
// Explanation: LPS is "ddd".

std::cout << 'Length of LPS ---> ' + findLPSLength('pqr'));
// Output: 1
// Explanation: LPS could be "p", "q" or "r".
```
...
public static Object findLPSLength(str) {
  public static Object findLPSLengthRecursive(str, startIndex, endIndex) {
    ...
    skipStartChar = findLPSLengthRecursive(str, startIndex + 1, endIndex);
    skipEndChar = findLPSLengthRecursive(str, startIndex, endIndex - 1);
...
auto findLPSLength(str) {
  auto findLPSLengthRecursive(str, startIndex, endIndex) {
    ...
    //case 1: elements at the start and the end are the same
    if (str[startIndex] === str[endIndex]) {
      auto remainingLength = endIndex - startIndex - 1;
    ...
    //case 2: skip one character either from the start or end
    auto skipStartChar = findLPSLengthRecursive(str, startIndex + 1, endIndex);
    auto skipEndChar = findLPSLengthRecursive(str, startIndex, endIndex - 1);
    return std::max(skipStartChar, skipEndChar);
  }
...
def findLPSLength(str):
    ...
    def findLPSLengthRecursive(str, startIndex, endIndex):
        ...
        skipStartChar = findLPSLengthRecursive(str, startIndex + 1, endIndex)
        skipEndChar = findLPSLengthRecursive(str, startIndex, endIndex - 1)
        dp[startIndex][endIndex] = max(skipStartChar, skipEndChar)
        return dp[startIndex][endIndex]
...
auto findLPSLength(str) {
  ...
  auto findLPSLengthRecursive(str, startIndex, endIndex) {
    ...
    auto skipStartChar = findLPSLengthRecursive(str, startIndex + 1, endIndex);
    auto skipEndChar = findLPSLengthRecursive(str, startIndex, endIndex - 1);
    dp[startIndex][endIndex] = std::max(skipStartChar, skipEndChar);
    return dp[startIndex][endIndex];
  }
...
### Java
```java
public static Object findCPS(str) {
...
System.out.println('Length of LPS: ---> ' + findCPS('abdbca'));
// Output: 7
// Explanation: Here are the palindromic substrings, "a", "b", "d", "b", "c", "a", "bdb".

System.out.println('Length of LPS: ---> ' + findCPS('cddpd'));
// Output: 7
// Explanation: Here are the palindromic substrings, "c", "d", "d", "p", "d", "dd", "dpd".

System.out.println('Length of LPS: ---> ' + findCPS('pqr'));
// Output: 3
// Explanation: Here are the palindromic substrings,"p", "q", "r".
```
...
print('Length of LPS: ---> ' + findCPS('abdbca'))
# Output: 7
# Explanation: Here are the palindromic substrings, "a", "b", "d", "b", "c", "a", "bdb".

print('Length of LPS: ---> ' + findCPS('cddpd'))
# Output: 7
# Explanation: Here are the palindromic substrings, "c", "d", "d", "p", "d", "dd", "dpd".

print('Length of LPS: ---> ' + findCPS('pqr'))
# Output: 3
# Explanation: Here are the palindromic substrings,"p", "q", "r".
```
...
std::cout << 'Length of LPS: ---> ' + findCPS('abdbca'));
// Output: 7
// Explanation: Here are the palindromic substrings, "a", "b", "d", "b", "c", "a", "bdb".

std::cout << 'Length of LPS: ---> ' + findCPS('cddpd'));
// Output: 7
// Explanation: Here are the palindromic substrings, "c", "d", "d", "p", "d", "dd", "dpd".

std::cout << 'Length of LPS: ---> ' + findCPS('pqr'));
// Output: 3
// Explanation: Here are the palindromic substrings,"p", "q", "r".
```
...
### Java
```java
public static Object findMinimumDeletions(str) {
  public static Object findLPSLength(str) {
    ...
    for (i = 0; i < str.length; i++) dp[i][i] = 1;
...
def findMinimumDeletions(str):
    def findLPSLength(str):
        ...
        for i in range(len(str)):
            ...
            #subtracting the length of the LPS from the length
            #of the input string to get minimum number of deletions
            return len(str) - findLPSLength(str)
...
auto findMinimumDeletions(str) {
  auto findLPSLength(str) {
    ...
    //every sequence with one element is a palindrome of length 1
    for (auto i = 0; i < str.size(); i++) dp[i][i] = 1;
  ...
  //subtracting the length of the LPS from the length
  //of the input string to get minimum number of deletions
  return str.size() - findLPSLength(str);
}
...
### Java
```java
public static Object minInsertions(s) {
  public static Object findLPSLength(s) {
    // dp[i][j] stores the length of LPS from index 'i' to index 'j'
    dp = Array(s.length)
      .fill(0)
    ...
    for (startIndex = s.length - 1; startIndex >= 0; startIndex--) {
...
def minInsertions(s):
    def findLPSLength(s):
        ...
        dp = Array(len(s))
        .fill(0)
        .map(() => Array(len(s)).fill(0))

        #every sequence with one element is a palindrome of length 1
        for i in range(len(s)):

            for (startIndex = len(s) - 1; startIndex >= 0; startIndex--):
...
### C++
```cpp
auto minInsertions(s) {
  auto findLPSLength(s) {
    // dp[i][j] stores the length of LPS from index 'i' to index 'j'
    auto dp = Array(s.size())
      .fill(0)
      .map(() => Array(s.size()).fill(0));

    //every sequence with one element is a palindrome of length 1
    for (auto i = 0; i < s.size(); i++) dp[i][i] = 1;

    for (auto startIndex = s.size() - 1; startIndex >= 0; startIndex--) {
      for (auto endIndex = startIndex + 1; endIndex < s.size(); endIndex++) {
        // case: 1 elements at the beggining and end are the same
        if (s.charAt(startIndex) === s.charAt(endIndex)) {
          dp[startIndex][endIndex] = 2 + dp[startIndex + 1][endIndex - 1];
        } else {
...
### Java
```java
public static Object isValidPalindrome(s, K) {
  public static Object findLPSLength(s) {
    // dp[i][j] stores the length of LPS from index 'i' to index 'j'
    dp = Array(s.length)
      .fill(0)
...
def isValidPalindrome(s, K):
    def findLPSLength(s):
        ...
        for i in range(len(s)):
            ...
            #subtracting the length of the LPS from the length
            #of the input string to get minimum number of deletions < k
            return len(s) - findLPSLength(s) <= k
...
auto isValidPalindrome(s, K) {
  ...
  //subtracting the length of the LPS from the length
  //of the input string to get minimum number of deletions < k
  return s.size() - findLPSLength(s) <= k;
}
...
### Java
```java
public static Object findMPPCuts(str) {
  public static Object findMPPCutsRecursive(str, startIndex, endIndex) {
  ...
  public static Object isPalindrome(str, start, end) {
    while (start <= end) {
...
System.out.println(`Minimum palindrome partitions ---> ${findMPPCuts('abdbca')}`);
// Output: 3
// Explanation: Palindrome pieces are "a", "bdb", "c", "a".

System.out.println(`Minimum palindrome partitions ---> ${findMPPCuts('cdpdd')}`);
// Output: 2
// Explanation: Palindrome pieces are "c", "d", "dpd".

System.out.println(`Minimum palindrome partitions ---> ${findMPPCuts('pqr')}`);
// Output: 2
// Explanation: Palindrome pieces are "p", "q", "r".

System.out.println(`Minimum palindrome partitions ---> ${findMPPCuts('pp')}`);
// Output: 0
// Explanation: We do not need to cut, as "pp" is a palindrome.
```
...
```python
def findMPPCuts(str):
    def findMPPCutsRecursive(str, startIndex, endIndex):
        #base case: we don't need to cut the str if it is a palindrom
        if startIndex >= endIndex || isPalindrome(str, startIndex, endIndex:)
        return 0
    ...
    return findMPPCutsRecursive(str, 0, len(str) - 1)

print(`Minimum palindrome partitions ---> ${findMPPCuts('abdbca')}`)
# Output: 3
# Explanation: Palindrome pieces are "a", "bdb", "c", "a".

print(`Minimum palindrome partitions ---> ${findMPPCuts('cdpdd')}`)
# Output: 2
# Explanation: Palindrome pieces are "c", "d", "dpd".

print(`Minimum palindrome partitions ---> ${findMPPCuts('pqr')}`)
# Output: 2
# Explanation: Palindrome pieces are "p", "q", "r".

print(`Minimum palindrome partitions ---> ${findMPPCuts('pp')}`)
# Output: 0
# Explanation: We do not need to cut, as "pp" is a palindrome.
```
...
### C++
```cpp
auto findMPPCuts(str) {
  auto findMPPCutsRecursive(str, startIndex, endIndex) {
    //base case: we don't need to cut the str if it is a palindrom
    if (startIndex >= endIndex || isPalindrome(str, startIndex, endIndex))
      return 0;

    //at most, we need to cut the str into it's length-1 pieces
    auto minimumCuts = endIndex - startIndex;
  ...
  auto isPalindrome(str, start, end) {
    while (start <= end) {
      if (str[start++] !== str[end--]) return false;
    }
    return true;
  ...
  return findMPPCutsRecursive(str, 0, str.size() - 1);
}

std::cout << `Minimum palindrome partitions ---> ${findMPPCuts('abdbca')}`);
// Output: 3
// Explanation: Palindrome pieces are "a", "bdb", "c", "a".

std::cout << `Minimum palindrome partitions ---> ${findMPPCuts('cdpdd')}`);
// Output: 2
// Explanation: Palindrome pieces are "c", "d", "dpd".

std::cout << `Minimum palindrome partitions ---> ${findMPPCuts('pqr')}`);
// Output: 2
// Explanation: Palindrome pieces are "p", "q", "r".

std::cout << `Minimum palindrome partitions ---> ${findMPPCuts('pp')}`);
// Output: 0
// Explanation: We do not need to cut, as "pp" is a palindrome.
```
...
def findMPPCuts(str):
    ...
    def isPalindrome(str, start, end):
        dpIsPalindrome[start] = dpIsPalindrome[start] || []
        if typeof dpIsPalindrome[start][end] === 'None':
            dpIsPalindrome[start][end] = True
            i = start
            j = end
            ...
            while i <= j:
                ...
                #use <i>memoization</i> to find if the remaining string is a palindrome
                dpIsPalindrome[i] = dpIsPalindrome[i] || []
                if i < j && typeof dpIsPalindrome[i][j] !== 'None':
                    dpIsPalindrome[start][end] = dpIsPalindrome[i][j]
                    break
...
System.out.println(`Minimum palindrome partitions ---> ${findMPPCuts('madam')}`);
// Output: 0
...
def findMPPCuts(str):
    ...
    #stores the min cuts needed for the substring
    #from that index until the end
    cuts = Array(len(str)).fill(0)

    for (startIndex = len(str) - 1; startIndex >= 0; startIndex--):
        #maximum cuts
        minCuts = len(str)

        for (endIndex = len(str) - 1; endIndex >= startIndex; endIndex--):
            if isPalindrome[startIndex][endIndex]:
                #we can cut here as we got a palindrome
                #also we dont need any cut if the whole substring is a palindrome
                minCuts =
...
print(`Minimum palindrome partitions ---> ${findMPPCuts('madam')}`)
# Output: 0
# Explanation: We do not need to cut, as "madam" is a palindrome.
```
...
auto findMPPCuts(str) {
  // isPalindrome[i][j] will be 'true' if the string from index 'i' to index 'j' is a palindrome
  auto isPalindrome = Array(str.size())
    .fill(false)
    .map(() => Array(str.size()).fill(false));
  ...
  //stores the min cuts needed for the substring
  //from that index until the end
  auto cuts = Array(str.size()).fill(0);

  for (auto startIndex = str.size() - 1; startIndex >= 0; startIndex--) {
    //maximum cuts
    auto minCuts = str.size();

    for (auto endIndex = str.size() - 1; endIndex >= startIndex; endIndex--) {
      if (isPalindrome[startIndex][endIndex]) {
        //we can cut here as we got a palindrome
        //also we dont need any cut if the whole substring is a palindrome
        minCuts =
          endIndex === str.size() - 1
            ? 0
            : std::min(minCuts, 1 + cuts[endIndex + 1]);
      }
    }
    cuts[startIndex] = minCuts;
  }
...
std::cout << `Minimum palindrome partitions ---> ${findMPPCuts('madam')}`);
// Output: 0
// Explanation: We do not need to cut, as "madam" is a palindrome.
```
...
### Java
```java
public static Object findLCSLength(str1, str2) {
  public static Object findLCSLengthRecursive(str1, str2, index1, index2, count) {
    //base case
    ...
    checkSecondString = findLCSLengthRecursive(
      str1,
      str2,
      index1 + 1,
      index2,
...
### Python
```python
def findLCSLength(str1, str2):
    def findLCSLengthRecursive(str1, str2, index1, index2, count):
        #base case
        if index1 === len(str1) || index2 === len(str2):return count
        ...
        return max(count, max(checkFirstString, checkSecondString))
    return findLCSLengthRecursive(str1, str2, 0, 0, 0)
...
### C++
```cpp
auto findLCSLength(str1, str2) {
  auto findLCSLengthRecursive(str1, str2, index1, index2, count) {
    //base case
    if (index1 === str1.size() || index2 === str2.size()) return count;
    ...
    auto checkFirstString = findLCSLengthRecursive(
      str1,
      str2,
      index1,
      index2 + 1,
    ...
    auto checkSecondString = findLCSLengthRecursive(
      str1,
      str2,
      index1 + 1,
      index2,
    ...
    return std::max(count, std::max(checkFirstString, checkSecondString));
  }
  return findLCSLengthRecursive(str1, str2, 0, 0, 0);
}
...
```java
public static Object findLCSLength(str1, str2) {
  maxLength = Math.min(str1.length, str2.length);
  dp = [];
  ...
  public static Object findLCSLengthRecursive(str1, str2, index1, index2, count) {
    ...
    if (typeof dp[index1][index2][count] === 'undefined') {
      ...
      checkThirdString = findLCSLengthRecursive(
        str1,
        str2,
        index1 + 1,
        index2,
...
def findLCSLength(str1, str2):
    ...
    def findLCSLengthRecursive(str1, str2, index1, index2, count):
        ...
        if typeof dp[index1][index2][count] === 'None':
            checkFirstString = count
            if str1[index1] === str2[index2]:
                checkFirstString = findLCSLengthRecursive(
                str1,
            ...
            dp[index1][index2][count] = max(
            checkFirstString,
            max(checkSecondString, checkThirdString)
            )
...
```cpp
auto findLCSLength(str1, str2) {
  auto maxLength = std::min(str1.size(), str2.size());
  auto dp = [];
  ...
  auto findLCSLengthRecursive(str1, str2, index1, index2, count) {
    ...
    if (typeof dp[index1][index2][count] === 'undefined') {
      ...
      auto checkThirdString = findLCSLengthRecursive(
        str1,
        str2,
        index1 + 1,
        index2,
      ...
      dp[index1][index2][count] = std::max(
        checkFirstString,
        std::max(checkSecondString, checkThirdString)
      );
    }
...
```cpp
auto findLCSLength(str1, str2) {
  auto dp = Array(str1.size() + 1)
    .fill(0)
    .map(() => Array(str2.size() + 1).fill(0));
  auto maxLength = 0;
...
```java
public static Object findLCSLength(str1, str2) {
  public static Object findLCSLengthRecursive(str1, str2, index1, index2) {
    // base check
...
```python
def findLCSLength(str1, str2):
    def findLCSLengthRecursive(str1, str2, index1, index2):
        # base check
        if index1 === len(str1) || index2 === len(str2):return 0
...
```cpp
auto findLCSLength(str1, str2) {
  auto findLCSLengthRecursive(str1, str2, index1, index2) {
    // base check
    ...
    return std::max(checkFirstString, checkSecondString);
  }
  return findLCSLengthRecursive(str1, str2, 0, 0);
}
...
def findLCSLength(str1, str2):
    ...
    def findLCSLengthRecursive(str1, str2, index1, index2):
        ...
        if typeof dp[index1][index2] === 'None':
            if str1[index1] === str2[index2]:
                dp[index1][index2] =
                1 + findLCSLengthRecursive(str1, str2, index1 + 1, index2 + 1)
                else:
                    ...
                    dp[index1][index2] = max(checkFirstString, checkSecondString)
            # console.log(dp);
            return dp[index1][index2]
        return findLCSLengthRecursive(str1, str2, 0, 0)
  ...
  auto findLCSLengthRecursive(str1, str2, index1, index2) {
    ...
    if (typeof dp[index1][index2] === 'undefined') {
      if (str1[index1] === str2[index2]) {
        ...
        dp[index1][index2] = std::max(checkFirstString, checkSecondString);
      }
    }
    // std::cout << dp);
    return dp[index1][index2];
...
auto findLCSLength(str1, str2) {
  ...
  for (auto index1 = 1; index1 <= str1.size(); index1++) {
    for (auto index2 = 1; index2 <= str2.size(); index2++) {
      if (str1[index1 - 1] === str2[index2 - 1]) {
        dp[index1][index2] = 1 + dp[index1 - 1][index2 - 1];
      } else {
        dp[index1][index2] = std::max(
          dp[index1 - 1][index2],
          dp[index1][index2 - 1]
        );
      }
      // std::cout << dp)
      maxLength = std::max(maxLength, dp[index1][index2]);
    }
  }
...
### Java
```java
public static Object findMDI(str1, str2) {
  c1 = findLCSLength(str1, str2);
...
def findMDI(str1, str2):
    ...
    def findLCSLength(str1, str2):
        ...
        print(
        `We need ${len(str1) - c1} deletions and $:
            len(str2) - c1
            } insertions to transform "${str1}" into "${str2}"`
            )
...
### C++
```cpp
auto findMDI(str1, str2) {
  auto c1 = findLCSLength(str1, str2);
  ...
  std::cout << 
    `We need ${str1.size() - c1} deletions and ${
      str2.size() - c1
    } insertions to transform "${str1}" into "${str2}"`
  );
}
...
### Java
```java
public static Object findLISLength(nums) {
  public static Object findLISLengthRecursive(nums, currIndex, prevIndex) {
    // base check
    ...
    count2 = findLISLengthRecursive(nums, currIndex + 1, prevIndex);
...
def findLISLength(nums):
    def findLISLengthRecursive(nums, currIndex, prevIndex):
        # base check
        if currIndex === len(nums):return 0
...
### C++
```cpp
auto findLISLength(nums) {
  auto findLISLengthRecursive(nums, currIndex, prevIndex) {
    // base check
    ...
    //exluding number at currIndex
    auto count2 = findLISLengthRecursive(nums, currIndex + 1, prevIndex);
...
def findLISLength(nums):
    ...
    def findLISLengthRecursive(nums, currIndex, prevIndex):
        ...
        if typeof dp[currIndex][prevIndex + 1] === 'None':
            #include nums[currIndex] if if is larger than the last included number
            count1 = 0
            ...
            #exluding number at currIndex
            count2 = findLISLengthRecursive(nums, currIndex + 1, prevIndex)
            dp[currIndex][prevIndex + 1] = max(count1, count2)
...
auto findLISLength(nums) {
  ...
  auto findLISLengthRecursive(nums, currIndex, prevIndex) {
    ...
    if (typeof dp[currIndex][prevIndex + 1] === 'undefined') {
      ...
      //exluding number at currIndex
      auto count2 = findLISLengthRecursive(nums, currIndex + 1, prevIndex);
      dp[currIndex][prevIndex + 1] = std::max(count1, count2);
    }
...
### Java
```java
public static Object findMSIS(nums) {
  public static Object findMSISRecursive(nums, currIndex, prevIndex, sum) {
    //base check
    if (currIndex === nums.length) return sum;

    //include nums[currIndex] if it is larger than the last include number
    sumIncludingCurrIndex = sum;
    ...
    sumWithoutCurrIndex = findMSISRecursive(
      nums,
      currIndex + 1,
      prevIndex,
      sum
...
```python
def findMSIS(nums):
    def findMSISRecursive(nums, currIndex, prevIndex, sum):
        #base check
        if currIndex === len(nums):return sum
        ...
        return max(sumIncludingCurrIndex, sumWithoutCurrIndex)
    return findMSISRecursive(nums, 0, -1, 0)
...
```cpp
auto findMSIS(nums) {
  auto findMSISRecursive(nums, currIndex, prevIndex, sum) {
    //base check
    if (currIndex === nums.size()) return sum;

    //include nums[currIndex] if it is larger than the last include number
    auto sumIncludingCurrIndex = sum;
    ...
    //exclude the number at currIndex
    auto sumWithoutCurrIndex = findMSISRecursive(
      nums,
      currIndex + 1,
      prevIndex,
      sum
    );
    return std::max(sumIncludingCurrIndex, sumWithoutCurrIndex);
  }
  return findMSISRecursive(nums, 0, -1, 0);
}
...
public static Object findMSIS(nums) {
  ...
  public static Object findMSISRecursive(nums, currIndex, prevIndex, sum) {
    ...
    subProbKey = `${currIndex}-${prevIndex}-${sum}`;
...
def findMSIS(nums):
    ...
    def findMSISRecursive(nums, currIndex, prevIndex, sum):
        ...
        if typeof dp[subProbKey] === 'None':
            ...
            # console.log(dp)
            dp[subProbKey] = max(sumIncludingCurrIndex, sumWithoutCurrIndex)
            return dp[subProbKey]
    return findMSISRecursive(nums, 0, -1, 0)
...
auto findMSIS(nums) {
  ...
  auto findMSISRecursive(nums, currIndex, prevIndex, sum) {
    ...
    auto subProbKey = `${currIndex}-${prevIndex}-${sum}`;
    ...
    if (typeof dp[subProbKey] === 'undefined') {
      ...
      // std::cout << dp)
      dp[subProbKey] = std::max(sumIncludingCurrIndex, sumWithoutCurrIndex);
      return dp[subProbKey];
    }
  }
  return findMSISRecursive(nums, 0, -1, 0);
...
### Java
```java
public static Object findSCSLength(s1, s2) {
  // **MY THOUGHT PROCESS**
  ...
  public static Object findSCSLengthRecursive(s1, s2, index1, index2) {
    ...
    length1 = 1 + findSCSLengthRecursive(s1, s2, index1, index2 + 1);
    length2 = 1 + findSCSLengthRecursive(s1, s2, index1 + 1, index2);
...
def findSCSLength(s1, s2):
    ...
    def findSCSLengthRecursive(s1, s2, index1, index2):
        #if we have reached the end of a string
        #return the remaining length of the other string
        #as in this case we have to take all of the remaining other string
        if index1 === len(s1):return len(s2) - index2
        if index2 === len(s2):return len(s1) - index1
...
auto findSCSLength(s1, s2) {
  ...
  auto findSCSLengthRecursive(s1, s2, index1, index2) {
    //if we have reached the end of a string
    //return the remaining length of the other string
    //as in this case we have to take all of the remaining other string
    if (index1 === s1.size()) return s2.size() - index2;
    if (index2 === s2.size()) return s1.size() - index1;

    if (s1[index1] === s2[index2])
      return 1 + findSCSLengthRecursive(s1, s2, index1 + 1, index2 + 1);

    auto length1 = 1 + findSCSLengthRecursive(s1, s2, index1, index2 + 1);
    auto length2 = 1 + findSCSLengthRecursive(s1, s2, index1 + 1, index2);
        ...
        if typeof dp[index1][index2] === 'None':
            if s1[index1] === s2[index2]:
                ...
                1 + findSCSLengthRecursive(s1, s2, index1 + 1, index2 + 1)
                    ...
                    dp[index1][index2] = min(length1, length2)
  ...
  auto findSCSLengthRecursive(s1, s2, index1, index2) {
    ...
    if (typeof dp[index1][index2] === 'undefined') {
      if (s1[index1] === s2[index2]) {
        ...
        dp[index1][index2] = std::min(length1, length2);
      }
    }
...
```cpp
auto findSCSLength(s1, s2) {
  auto dp = Array(s1.size() + 1)
    .fill(0)
    .map(() => Array(s2.size() + 1).fill(0));
...
### Java
```java
public static Object findMinimumDeletions(nums) {
  ...
  public static Object findLISLength(nums) {
    ...
    for (i = 1; i < nums.length; i++) {
      dp[i] = 1;
      for (j = 0; j < i; j++) {
        // System.out.println(dp[i], i, dp[j], j)
        if (nums[i] > nums[j] && dp[i] <= dp[j]) {
          dp[i] = dp[j] + 1;
          maxLength = Math.max(maxLength, dp[i]);
          // System.out.println(dp)
...
def findMinimumDeletions(nums):
    ...
    # substracting the length of the LIS from the length of the input array to
    # get the minimum number of deletions
    return len(nums) - findLISLength(nums)

    def findLISLength(nums):
        dp = Array(len(nums)).fill(0)
...
### C++
```cpp
auto findMinimumDeletions(nums) {
  /* Given a number sequence, find the minimum number of elements
   that should be deleted to make the remaining sequence sorted. */
  ...
  auto findLISLength(nums) {
    auto dp = Array(nums.size()).fill(0);
...
### Java
```java
public static Object findLRSLength(str) {
  ...
  public static Object findLRSLengthRecursive(str, index1, index2) {
...
def findLRSLength(str):
    ...
    def findLRSLengthRecursive(str, index1, index2):
        #base case => 0 length or end of str
        if index1 === len(str) || index2 === len(str):return 0
...
auto findLRSLength(str) {
  ...
  auto findLRSLengthRecursive(str, index1, index2) {
    //base case => 0 length or end of str
    if (index1 === str.size() || index2 === str.size()) return 0;

    if (index1 !== index2 && str[index1] === str[index2])
      return 1 + findLRSLengthRecursive(str, index1 + 1, index2 + 1);

    auto call1 = findLRSLengthRecursive(str, index1 + 1, index2);
    auto call2 = findLRSLengthRecursive(str, index1, index2 + 1);

    return std::max(call1, call2);
  }
...
auto findLRSLength(str) {
  ...
  auto findLRSLengthRecursive(str, index1, index2) {
    ...
    if (index1 !== index2 && str[index1] === str[index2])
      ...
      auto call1 = findLRSLengthRecursive(str, index1 + 1, index2);
      auto call2 = findLRSLengthRecursive(str, index1, index2 + 1);
      dp[index1][index2] = std::max(call1, call2);
    }
...
def findLRSLength(str):
    ...
    for index1 in range(1, = len(str)):
        for index2 in range(1, = len(str)):
            if index1 !== index2 && str[index1 - 1] === str[index2 - 1]:
                dp[index1][index2] = 1 + dp[index1 - 1][index2 - 1]
                else:
                    dp[index1][index2] = max(
...
```cpp
auto findLRSLength(str) {
  auto dp = Array(str.size() + 1)
    .fill(0)
    .map(() => Array(str.size() + 1).fill(0));
  ...
  for (auto index1 = 1; index1 <= str.size(); index1++) {
    for (auto index2 = 1; index2 <= str.size(); index2++) {
      if (index1 !== index2 && str[index1 - 1] === str[index2 - 1]) {
        dp[index1][index2] = 1 + dp[index1 - 1][index2 - 1];
      } else {
        dp[index1][index2] = std::max(
...
### Java
```java
public static Object findSPMCount(str, pattern) {
  public static Object findSPMCountRecursive(str, pattern, strIndex, patternIndex) {
...
### Python
```python
def findSPMCount(str, pattern):
    def findSPMCountRecursive(str, pattern, strIndex, patternIndex):
        # base case => if we have reached the end of the pattern
        if patternIndex === len(pattern):return 1

        # base case => if we have reached the end of the str
        # but pattern has some chars left
        if strIndex === len(str):return 0
...
### C++
```cpp
auto findSPMCount(str, pattern) {
  auto findSPMCountRecursive(str, pattern, strIndex, patternIndex) {
    // base case => if we have reached the end of the pattern
    if (patternIndex === pattern.size()) return 1;

    // base case => if we have reached the end of the str
    // but pattern has some chars left
    if (strIndex === str.size()) return 0;
    ...
    auto count2 = findSPMCountRecursive(
      str,
      pattern,
      strIndex + 1,
      patternIndex
...
public static Object findSPMCount(str, pattern) {
  ...
  for (i = 0; i <= str.length; i++) dp[i][0] = 1;

  for (strIndex = 1; strIndex <= str.length; strIndex++) {
...
def findSPMCount(str, pattern):
    ...
    if len(str) === 0 || len(pattern) > len(str):return 0

    # dp[strIndex][patIndex] will be storing the count of SPM up to str[0..strIndex-1][0..patIndex-1]
    dp = Array(len(str) + 1)
    .fill(0)
    .map(() => Array(len(pattern) + 1).fill(0))

    # for the empty pattern, we have one matching
    for i in range(= len(str)):

        for strIndex in range(1, = len(str)):
            for patternIndex in range(1, = len(pattern)):
                if str[strIndex - 1] === pattern[patternIndex - 1]:
                    dp[strIndex][patternIndex] = dp[strIndex - 1][patternIndex - 1]
                dp[strIndex][patternIndex] += dp[strIndex - 1][patternIndex]
...
auto findSPMCount(str, pattern) {
  ...
  if (str.size() === 0 || pattern.size() > str.size()) return 0;

  // dp[strIndex][patIndex] will be storing the count of SPM up to str[0..strIndex-1][0..patIndex-1]
  auto dp = Array(str.size() + 1)
    .fill(0)
    .map(() => Array(pattern.size() + 1).fill(0));

  // for the empty pattern, we have one matching
  for (auto i = 0; i <= str.size(); i++) dp[i][0] = 1;

  for (auto strIndex = 1; strIndex <= str.size(); strIndex++) {
    for (auto patternIndex = 1; patternIndex <= pattern.size(); patternIndex++) {
      if (str[strIndex - 1] === pattern[patternIndex - 1]) {
        dp[strIndex][patternIndex] = dp[strIndex - 1][patternIndex - 1];
      }
      dp[strIndex][patternIndex] += dp[strIndex - 1][patternIndex];
...
### Java
```java
public static Object findLBSLength(nums) {
  maxLength = 0;

  for (i = 0; i < nums.length; i++) {
    checkIndexToEnd = findLDSLength(nums, i, -1);
    checkIndexToStart = findLDSLengthReverse(nums, i, -1);
    // System.out.println(checkIndexToEnd, checkIndexToStart, checkIndexToEnd + checkIndexToStart - 1)
    maxLength = Math.max(maxLength, checkIndexToEnd + checkIndexToStart - 1);
  }
  return maxLength;
}

public static Object findLDSLength(nums, currIndex, prevIndex) {
  ...
  checkExcludingCurrIndex = findLDSLength(nums, currIndex + 1, prevIndex);

  return Math.max(checkWithCurrIndex, checkExcludingCurrIndex);
}

public static Object findLDSLengthReverse(nums, currIndex, prevIndex) {
  //find the longest decreasing subsequence from
  ...
  //excluding the number at currIndex
  checkExcludingCurrIndex = findLDSLengthReverse(
    nums,
    currIndex - 1,
    prevIndex
  );
...
def findLDSLength(nums, currIndex, prevIndex):
    #find the longest decreasing subsequence from
    #currIndex to the end of the array
    if currIndex === len(nums):return 0
...
def findLDSLengthReverse(nums, currIndex, prevIndex):
    #find the longest decreasing subsequence from
    #currIndex to the beginning of the array
    if currIndex < 0:return 0
...
### C++
```cpp
auto findLBSLength(nums) {
  auto maxLength = 0;

  for (auto i = 0; i < nums.size(); i++) {
    auto checkIndexToEnd = findLDSLength(nums, i, -1);
    auto checkIndexToStart = findLDSLengthReverse(nums, i, -1);
    // std::cout << checkIndexToEnd, checkIndexToStart, checkIndexToEnd + checkIndexToStart - 1)
    maxLength = std::max(maxLength, checkIndexToEnd + checkIndexToStart - 1);
  }
  return maxLength;
}

auto findLDSLength(nums, currIndex, prevIndex) {
  //find the longest decreasing subsequence from
  //currIndex to the end of the array
  if (currIndex === nums.size()) return 0;

  //include nums[currIndex] if it is smaller than previous number
  auto checkWithCurrIndex = 0;
  ...
  //excluding the number at currIndex
  auto checkExcludingCurrIndex = findLDSLength(nums, currIndex + 1, prevIndex);

  return std::max(checkWithCurrIndex, checkExcludingCurrIndex);
}

auto findLDSLengthReverse(nums, currIndex, prevIndex) {
  //find the longest decreasing subsequence from
  //currIndex to the beginning of the array
  if (currIndex < 0) return 0;
  ...
  //excluding the number at currIndex
  auto checkExcludingCurrIndex = findLDSLengthReverse(
    nums,
    currIndex - 1,
    prevIndex
  );
...
public static Object findLBSLength(nums) {
  ...
  public static Object findLDSLength(nums, currIndex, prevIndex) {
    ...
    if (typeof lds[currIndex][prevIndex + 1] === 'undefined') {
      ...
      //excluding the number at currIndex
      checkExcludingCurrIndex = findLDSLength(
        nums,
        currIndex + 1,
        prevIndex
      );
...
def findLBSLength(nums):
    ...
    def findLDSLength(nums, currIndex, prevIndex):
        ...
        if typeof lds[currIndex][prevIndex + 1] === 'None':
            #include nums[currIndex] if it is smaller than the previous Number
    ...
    def findLDSLengthReverse(nums, currIndex, prevIndex):
        ...
        if ldsReversed[currIndex][prevIndex + 1] == None:
            # include nums[currIndex] if it is smaller than the prev number
            checkWithCurrIndex = 0
            ...
            ldsReversed[currIndex][prevIndex + 1] = max(
            checkWithCurrIndex,
            checkExcludingCurrIndex
            )
...
auto findLBSLength(nums) {
  ...
  auto findLDSLength(nums, currIndex, prevIndex) {
    ...
    if (typeof lds[currIndex][prevIndex + 1] === 'undefined') {
      ...
      //excluding the number at currIndex
      auto checkExcludingCurrIndex = findLDSLength(
        nums,
        currIndex + 1,
        prevIndex
      );

      lds[currIndex][prevIndex + 1] = std::max(
        checkWithCurrIndex,
        checkExcludingCurrIndex
      );
    }
  ...
  auto findLDSLengthReverse(nums, currIndex, prevIndex) {
    ...
    if (ldsReversed[currIndex][prevIndex + 1] == nullptr) {
      // include nums[currIndex] if it is smaller than the prev number
      auto checkWithCurrIndex = 0;
      ...
      ldsReversed[currIndex][prevIndex + 1] = std::max(
        checkWithCurrIndex,
        checkExcludingCurrIndex
      );
    }
...
```java
public static Object findLBSLength(nums) {
  lds = Array(nums.length).fill(0);
  ldsReversed = Array(nums.length).fill(0);
...
```python
def findLBSLength(nums):
    lds = Array(len(nums)).fill(0)
    ldsReversed = Array(len(nums)).fill(0)
    ...
    for i in range(len(nums)):
        maxLength = max(maxLength, ldsReversed[i] + ldsReversed[i] - 1)
...
```cpp
auto findLBSLength(nums) {
  auto lds = Array(nums.size()).fill(0);
  auto ldsReversed = Array(nums.size()).fill(0);
  ...
  for (auto i = nums.size() - 1; i >= 0; i--) {
    ...
    for (auto j = i + 1; j < nums.size(); j++) {
      if (nums[j] < nums[i]) {
        ldsReversed[i] = std::max(ldsReversed[i], ldsReversed[j] + 1);
      }
    }
  }
  ...
  for (auto i = 0; i < nums.size(); i++) {
    maxLength = std::max(maxLength, ldsReversed[i] + ldsReversed[i] - 1);
  }
...
### Java
```java
public static Object findLASLength(nums) {
  public static Object findLASLengthRecursive(nums, prevIndex, currIndex, isAscending) {
    if (currIndex === nums.length) return 0;
    ...
    condition2 = findLASLengthRecursive(
      nums,
      prevIndex,
      currIndex + 1,
      isAscending
...
System.out.println(`Length of Longest Alternating Subsequence: --->
...
### Python
```python
def findLASLength(nums):
    def findLASLengthRecursive(nums, prevIndex, currIndex, isAscending):
        if currIndex === len(nums):return 0
    ...
    print(`Length of Longest Alternating Subsequence: --->
    ${findLASLength([1, 2, 3, 4])}`)
    #Output: 2
    # Explanation: There are many LAS: {1,2}, {3,4}, {1,3}, {1,4}
...
### C++
```cpp
auto findLASLength(nums) {
  auto findLASLengthRecursive(nums, prevIndex, currIndex, isAscending) {
    if (currIndex === nums.size()) return 0;
    ...
    //OR skip the current element
    auto condition2 = findLASLengthRecursive(
      nums,
      prevIndex,
      currIndex + 1,
      isAscending
    );
    return std::max(condition1, condition2);
  }
...
std::cout << `Length of Longest Alternating Subsequence: --->
${findLASLength([1, 2, 3, 4])}`);
//Output: 2
// Explanation: There are many LAS: {1,2}, {3,4}, {1,3}, {1,4}
...
def findLASLength(nums):
    ...
    def findLASLengthRecursive(nums, prevIndex, currIndex, isAscending):
        ...
        if (
        typeof dp[prevIndex + 1][currIndex][isAscending ? 1 : 0] === 'None'
        ):
            condition1 = 0
            #if ascending, the next element should be bigger
            if isAscending:
                ...
                dp[prevIndex + 1][currIndex][isAscending ? 1 : 0] = max(
                condition1,
                condition2
                )
...
auto findLASLength(nums) {
  ...
  auto findLASLengthRecursive(nums, prevIndex, currIndex, isAscending) {
    ...
    if (
      ...
      dp[prevIndex + 1][currIndex][isAscending ? 1 : 0] = std::max(
        condition1,
        condition2
      );
    }
...
### Java
```java
public static Object findMinOperations(s1, s2) {
  public static Object findMinOperationsRecursive(s1, s2, index1, index2) {
    ...
    //perform deletion
    onDeletion = 1 + findMinOperationsRecursive(s1, s2, index1 + 1, index2);
    //perform insertion
    onInsertion =
...
System.out.println(`Minimum Edit Distance: ---> ${findMinOperations('bat', 'but')}`);
// Output: 1
// Explanation: We just need to replace 'a' with 'u' to transform s1 to s2.
...
### Python
```python
def findMinOperations(s1, s2):
    def findMinOperationsRecursive(s1, s2, index1, index2):
        # if we have reached the end of s1, then we have to insert all the remaining characters of s2
        if index1 == len(s1):return len(s2) - index2
...
print(`Minimum Edit Distance: ---> ${findMinOperations('bat', 'but')}`)
# Output: 1
# Explanation: We just need to replace 'a' with 'u' to transform s1 to s2.
...
### C++
```cpp
auto findMinOperations(s1, s2) {
  auto findMinOperationsRecursive(s1, s2, index1, index2) {
    // if we have reached the end of s1, then we have to insert all the remaining characters of s2
    if (index1 == s1.size()) return s2.size() - index2;
    ...
    //perform deletion
    auto onDeletion = 1 + findMinOperationsRecursive(s1, s2, index1 + 1, index2);
    //perform insertion
    auto onInsertion =
      1 + findMinOperationsRecursive(s1, s2, index1, index2 + 1);
    // perform replacement
    ...
    return std::min(onDeletion, std::min(onInsertion, onReplacement));
  }
  return findMinOperationsRecursive(s1, s2, 0, 0);
}

std::cout << `Minimum Edit Distance: ---> ${findMinOperations('bat', 'but')}`);
// Output: 1
// Explanation: We just need to replace 'a' with 'u' to transform s1 to s2.
  ...
  public static Object findMinOperationsRecursive(s1, s2, index1, index2) {
    ...
    if (typeof dp[index1][index2] === 'undefined') {
      ...
      // If the strings have a matching character, we can recursively match for the remaining lengths.
        ...
        //perform deletion
        onDeletion = findMinOperationsRecursive(
          s1,
          s2,
          index1 + 1,
          index2
        ...
        //perform insertion
        onInsertion = findMinOperationsRecursive(
          s1,
          s2,
          index1,
          index2 + 1
        ...
        // perform replacement
        onReplacement = findMinOperationsRecursive(
          s1,
          s2,
          index1 + 1,
          index2 + 1
...
def findMinOperations(s1, s2):
    ...
    def findMinOperationsRecursive(s1, s2, index1, index2):
        ...
        if typeof dp[index1][index2] === 'None':
            # if we have reached the end of s1, then we have to insert all the remaining characters of s2
            if index1 === len(s1):dp[index1][index2] = len(s2) - index2
            # if we have reached the end of s2, then we have to delete all the remaining characters of s1
            else if index2 == len(s2):dp[index1][index2] = len(s1) - index1
            # If the strings have a matching character, we can recursively match for the remaining lengths.
            else if s1[index1] === s2[index2]:
                dp[index1][index2] = findMinOperationsRecursive(
                s1,
                ...
                index2 + 1
                    ...
                    dp[index1][index2] =
                    1 + min(onDeletion, min(onInsertion, onReplacement))
...
auto findMinOperations(s1, s2) {
  ...
  auto findMinOperationsRecursive(s1, s2, index1, index2) {
    ...
    if (typeof dp[index1][index2] === 'undefined') {
      // if we have reached the end of s1, then we have to insert all the remaining characters of s2
      if (index1 === s1.size()) dp[index1][index2] = s2.size() - index2;
      // if we have reached the end of s2, then we have to delete all the remaining characters of s1
      else if (index2 == s2.size()) dp[index1][index2] = s1.size() - index1;
      // If the strings have a matching character, we can recursively match for the remaining lengths.
      else if (s1[index1] === s2[index2])
        dp[index1][index2] = findMinOperationsRecursive(
          s1,
        ...
        //perform deletion
        auto onDeletion = findMinOperationsRecursive(
          s1,
          s2,
          index1 + 1,
          index2
        ...
        //perform insertion
        auto onInsertion = findMinOperationsRecursive(
          s1,
          s2,
          index1,
          index2 + 1
        ...
        // perform replacement
        auto onReplacement = findMinOperationsRecursive(
          s1,
          s2,
          index1 + 1,
          index2 + 1
        ...
        dp[index1][index2] =
          1 + std::min(onDeletion, std::min(onInsertion, onReplacement));
      }
    }
...
public static Object findMinOperations(s1, s2) {
  ...
  for (index1 = 0; index1 <= s1.length; index1++) dp[index1][0] = index1;

  // if s1 is empty, we have to insert all the characters of s2
  for (index2 = 0; index2 <= s2.length; index2++) dp[0][index2] = index2;
...
def findMinOperations(s1, s2):
    ...
    for index1 in range(= len(s1)):
        ...
        for index2 in range(= len(s2)):
            ...
            for index1 in range(1, = len(s1)):
                for index2 in range(1, = len(s2)):
                    # If the strings have a matching character, we can recursively match for the remaining lengths
                    if s1[index1 - 1] === s2[index2 - 1]:
                        dp[index1][index2] = dp[index1 - 1][index2 - 1]
                        else:
                            ...
                            min(
                            dp[index1 - 1][index2],
                            min(dp[index1][index2 - 1], dp[index1 - 1][index2 - 1])
                            )
...
auto findMinOperations(s1, s2) {
  ...
  // if s2 is empty, we can remove all the characters of s1 to make it empty too
  for (auto index1 = 0; index1 <= s1.size(); index1++) dp[index1][0] = index1;

  // if s1 is empty, we have to insert all the characters of s2
  for (auto index2 = 0; index2 <= s2.size(); index2++) dp[0][index2] = index2;

  for (auto index1 = 1; index1 <= s1.size(); index1++) {
    for (auto index2 = 1; index2 <= s2.size(); index2++) {
      // If the strings have a matching character, we can recursively match for the remaining lengths
      if (s1[index1 - 1] === s2[index2 - 1]) {
        dp[index1][index2] = dp[index1 - 1][index2 - 1];
      } else {
        dp[index1][index2] =
          ...
          std::min(
            dp[index1 - 1][index2],
            std::min(dp[index1][index2 - 1], dp[index1 - 1][index2 - 1])
          );
      }
    }
  }
...
### Java
```java
public static Object findSI(m, n, p) {
  public static Object findSIRecursive(m, n, p, mIndex, nIndex, pIndex) {
...
System.out.println(`String leterleaving: ---> ${findSI('abd', 'cef', 'abcdef')}`);
// Output: true
// Explanation: 'p' contains all the letters from 'm' and 'n' and preserves their order too.

System.out.println(`String leterleaving: ---> ${findSI('abd', 'cef', 'adcbef')}`);
// Output: false
// Explanation: 'p' contains all the letters from 'm' and 'n' but does not preserve the order.

System.out.println(`String leterleaving: ---> ${findSI('abc', 'def', 'abdccf')}`);
// Output: false
// Explanation: 'p' does not contain all the letters from 'm' and 'n'.
...
```python
def findSI(m, n, p):
    def findSIRecursive(m, n, p, mIndex, nIndex, pIndex):
        # if we have reached the end of the all the strings
        if mIndex === len(m) && nIndex === len(n) && pIndex === len(p):
            return True

            # if we have reached the end of 'p' but 'm' or 'n' still have some characters left
            if pIndex === len(p):return False

            mMatchesP = False
            nMatchesP = False

            if mIndex < len(m) && m[mIndex] === p[pIndex]:
                mMatchesP = findSIRecursive(m, n, p, mIndex + 1, nIndex, pIndex + 1)

                if nIndex < len(n) && n[nIndex] === p[pIndex]:
                    nMatchesP = findSIRecursive(m, n, p, mIndex, nIndex + 1, pIndex + 1)

                    return mMatchesP || nMatchesP
                return findSIRecursive(m, n, p, 0, 0, 0)

            print(`String leterleaving: ---> ${findSI('abd', 'cef', 'abcdef')}`)
            # Output: true
            # Explanation: 'p' contains all the letters from 'm' and 'n' and preserves their order too.

            print(`String leterleaving: ---> ${findSI('abd', 'cef', 'adcbef')}`)
            # Output: false
            # Explanation: 'p' contains all the letters from 'm' and 'n' but does not preserve the order.

            print(`String leterleaving: ---> ${findSI('abc', 'def', 'abdccf')}`)
            # Output: false
            # Explanation: 'p' does not contain all the letters from 'm' and 'n'.
...
```cpp
auto findSI(m, n, p) {
  auto findSIRecursive(m, n, p, mIndex, nIndex, pIndex) {
    // if we have reached the end of the all the strings
    if (mIndex === m.size() && nIndex === n.size() && pIndex === p.size())
      return true;

    // if we have reached the end of 'p' but 'm' or 'n' still have some characters left
    if (pIndex === p.size()) return false;

    auto mMatchesP = false;
    auto nMatchesP = false;
    
    if (mIndex < m.size() && m[mIndex] === p[pIndex])
      mMatchesP = findSIRecursive(m, n, p, mIndex + 1, nIndex, pIndex + 1);

    if (nIndex < n.size() && n[nIndex] === p[pIndex])
      nMatchesP = findSIRecursive(m, n, p, mIndex, nIndex + 1, pIndex + 1);
...
std::cout << `String leterleaving: ---> ${findSI('abd', 'cef', 'abcdef')}`);
// Output: true
// Explanation: 'p' contains all the letters from 'm' and 'n' and preserves their order too.

std::cout << `String leterleaving: ---> ${findSI('abd', 'cef', 'adcbef')}`);
// Output: false
// Explanation: 'p' contains all the letters from 'm' and 'n' but does not preserve the order.

std::cout << `String leterleaving: ---> ${findSI('abc', 'def', 'abdccf')}`);
// Output: false
// Explanation: 'p' does not contain all the letters from 'm' and 'n'.
...
public static Object findSI(m, n, p) {
  ...
  public static Object findSIRecursive(m, n, p, mIndex, nIndex, pIndex) {
    ...
    subProblemKey = `${mIndex}-${nIndex}-${pIndex}`;
...
def findSI(m, n, p):
    ...
    def findSIRecursive(m, n, p, mIndex, nIndex, pIndex):
        ...
        if mIndex === len(m) && nIndex === len(n) && pIndex === len(p):
            ...
            subProblemKey = `${mIndex}-${nIndex}-${pIndex}`
            if typeof dp[subProblemKey] === 'None':
                mMatchesP = False
                nMatchesP = False
...
auto findSI(m, n, p) {
  ...
  auto findSIRecursive(m, n, p, mIndex, nIndex, pIndex) {
    ...
    auto subProblemKey = `${mIndex}-${nIndex}-${pIndex}`;
    if (typeof dp[subProblemKey] === 'undefined') {
      auto mMatchesP = false;
      auto nMatchesP = false;
...
public static Object findSI(m, n, p) {
  ...
  for (mIndex = 0; mIndex <= m.length; mIndex++) {
...
def findSI(m, n, p):
    ...
    dp = Array(len(m) + 1)
    .fill(False)
    .map(() => Array(len(n) + 1).fill(False))

    # make sure if lengths of the strings add up
    if len(m) + len(n) != len(p):return False

    for mIndex in range(= len(m)):
...
auto findSI(m, n, p) {
  // dp[mIndex][nIndex] will be storing the result of string leterleaving
  // up to p[0..mIndex+nIndex-1]
  auto dp = Array(m.size() + 1)
    .fill(false)
    .map(() => Array(n.size() + 1).fill(false));

  // make sure if lengths of the strings add up
  if (m.size() + n.size() != p.size()) return false;

  for (auto mIndex = 0; mIndex <= m.size(); mIndex++) {
    for (auto nIndex = 0; nIndex <= n.size(); nIndex++) {
      // if 'm' and 'n' are empty, then 'p' must have been empty too.
      if (mIndex === 0 && nIndex === 0) {
        dp[mIndex][nIndex] = true;
      }
