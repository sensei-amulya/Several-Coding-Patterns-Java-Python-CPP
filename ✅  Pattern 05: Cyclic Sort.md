...
### Java
```java
public static Object cyclicSort(nums) {
  i = 0;

  while (i < nums.length) {
    j = nums[i] - 1; //nums[i] = 3, 3-1 = 2
    if (nums[i] !== nums[j]) {
      //3 !== 2
      //swap
...
### C++
```cpp
auto cyclicSort(nums) {
  auto i = 0;

  while (i < nums.size()) {
    auto j = nums[i] - 1; //nums[i] = 3, 3-1 = 2
    if (nums[i] !== nums[j]) {
      //3 !== 2
      //swap
      // [nums[i], nums[j]] = [nums[j], nums[i]]
...
### Java
```java
public static Object findMissingNumber(nums) {
  i = 0;
  n = nums.length;
...
### Java
```java
public static Object findMissingNumbers(nums) {
  i = 0;
...
### Java
```java
public static Object findDuplicate(nums) {
  i = 0;
...
public static Object findStart(nums, cycleLength) {
  pointer1 = nums[0];
  pointer2 = nums[0];
...
def findStart(nums, cycleLength):
    pointer1 = nums[0]
    pointer2 = nums[0]
    #move pointer2 ahead by cycleLength steps
    while cycleLength > 0:
...
auto findDuplicate(nums) {
  //using fast & slow pointer method
  auto slow = nums[0];
  auto fast = nums[nums[0]];
  while (slow !== fast) {
    slow = nums[slow];
    fast = nums[nums[fast]];
  }
  //find the cycle length
  auto current = nums[nums[slow]];
  auto cycleLength = 1;
  while (current !== nums[slow]) {
    current = nums[current];
    cycleLength++;
...
auto findStart(nums, cycleLength) {
  auto pointer1 = nums[0];
  auto pointer2 = nums[0];
  //move pointer2 ahead by cycleLength steps
  while (cycleLength > 0) {
    pointer2 = nums[pointer2];
    cycleLength--;
...
### Java
```java
public static Object findAllDuplicates(nums) {
  i = 0;
...
### Java
```java
public static Object findCorruptNumbers(nums) {
  i = 0;
...
### Java
```java
public static Object findFirstSmallestMissingPositive(nums) {
...
### Python
```python
def findFirstSmallestMissingPositive(nums):
    #try to sort the array
    i = 0
    n = len(nums)
...
### C++
```cpp
auto findFirstSmallestMissingPositive(nums) {
  //try to sort the array
  auto i = 0;
  auto n = nums.size();
...
### Java
```java
public static Object findFirstKMissingPositive(nums, k) {
...
### C++
```cpp
auto findFirstKMissingPositive(nums, k) {
  //sort? the input array
  auto i = 0;
  auto n = nums.size();
