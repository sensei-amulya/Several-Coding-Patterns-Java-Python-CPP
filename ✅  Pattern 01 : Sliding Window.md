...
### Java
```java
public static Object findAvgOfSubarrays(arr, K) {
  results = []
...
### Java
```java
public static Object findAveragesOfSubarrays(arr, k) {
...
### C++
```cpp
auto findAveragesOfSubarrays(arr, k) {
  //sliding window approach
...
### Java
```java
public static Object maxSubarrayOfSizeK(arr, k) {
  //brute force
  maxSum = 0
  windowSum = 0
...
### Java
```java
public static Object smallestSubarrayWithGivenSum(arr, s) {
...
### C++
```cpp
auto smallestSubarrayWithGivenSum(arr, s) {
  //sliding window, BUT the window size is not fixed
  auto windowSum = 0
...
### Java
```java
public static Object longestSubstringWithKdistinct(str, k) {
...
### C++
```cpp
auto longestSubstringWithKdistinct(str, k) {
   // Given a string, find the length of the longest substring in it with no more than K distinct characters.
  auto windowStart = 0
  auto maxLength = 0
  auto charFrequency = {}
  ...
  //in the following loop we'll try to extend the range [windowStart, windowEnd]
  for(auto windowEnd = 0; windowEnd < str.size(); windowEnd++) {
    auto endChar = str[windowEnd]
    if(!(endChar in charFrequency)) {
      charFrequency[endChar] = 0
    }
    charFrequency[endChar]++
    ...
    while(Object.keys(charFrequency).size() > k) {
      //insert characters from the beginning of the string until we have 'K' distinct characters in the hashMap 
    //these characters will consitutue our sliding window.  We are asked to find the longest such window having no more that K distinct characters.  We will remember the length of the window as the longest window so far
    //we will keep adding on character in the sliding window in a stepwise fashion
      //in each step we will try to shrink the window from the beginning if the count of distinct characters in the hashmap is larger than K. We will shrink the window until we have no more that K distinct characters in the HashMap
      auto startChar = str[windowStart]
      charFrequency[startChar]--
      //while shrinking , we will decrement the characters frequency going out of the window and remove it from the HashMap if it's frequency becomes zero
      if(charFrequency[startChar] === 0) {
        delete charFrequency[startChar]
...
### Java
```java
public static Object totalFruit (fruits) {
  windowStart = 0
  windowMax = 0
  fruitMap = new Map()
...
def totalFruit(fruits):
    ...
    for windowEnd in range(len(fruits)):
        ...
        windowMax = max(windowMax, windowEnd - windowStart + 1)
...
### C++
```cpp
auto totalFruit (fruits) {
  auto windowStart = 0
  auto windowMax = 0
  auto fruitMap = new Map()

  
  //1. try to extend the window range
  for(auto windowEnd = 0; windowEnd < fruits.size(); windowEnd++) {
    auto endFruit = fruits[windowEnd]
    ...
    //2. Shrink the sliding window, until we are left with 2 fruits in the fruitMap
    while(fruitMap.size > 2) {
      auto startFruit = fruits[windowStart]
    ...
    windowMax = std::max(windowMax, windowEnd - windowStart + 1)
  }
...
### Java
```java
public static Object fruitsInBaskets(fruits) {
  windowStart = 0; 
  maxLength = 0; 
  fruitFrequency = {};
  
  //try to extend the range
  for(windowEnd = 0; windowEnd < fruits.length; window++) {
    endFruit = fruits[windowEnd]
...
def fruitsInBaskets(fruits):
    ...
    #try to extend the range
    for(windowEnd = 0; windowEnd < len(fruits); window++):
        endFruit = fruits[windowEnd]
        if !(endFruit in fruitFrequency:):
            fruitFrequency[endFruit] = 0
        fruitFrequency[endFruit]++
...
### C++
```cpp
auto fruitsInBaskets(fruits) {
  auto windowStart = 0; 
  auto maxLength = 0; 
  auto fruitFrequency = {};
  
  //try to extend the range
  for(auto windowEnd = 0; windowEnd < fruits.size(); window++) {
    auto endFruit = fruits[windowEnd]
    if(!(endFruit in fruitFrequency)) {
      fruitFrequency[endFruit] = 0
    }
    ...
    //shrink the sliding window, until we are left with '2' fruits in the fruitFrequency hashMap
    while(Object.keys(fruitFrequency).size() > 2) {
      auto startFruit = fruits[windowStart];
      fruitFrequency[startFruit]--
      if(fruitFrequency[startFruit] === 0) {
        delete fruitFrequency[startFruit]
...
### Java
```java
public static Object lengthOfLongestSubstringTwoDistinct(s) {
    windowStart = 0
    maxLength = 0
    charFreq = {}
...
### C++
```cpp
auto lengthOfLongestSubstringTwoDistinct(s) {
    auto windowStart = 0
    auto maxLength = 0
    auto charFreq = {}
  
    //try to extend the range
    for(auto windowEnd = 0; windowEnd < s.size(); windowEnd++) {
        auto endChar = s[windowEnd]
      ...
      while(Object.keys(charFreq).size() > 2) {
        auto startChar = s[windowStart]
        charFreq[startChar]--
        if(charFreq[startChar] === 0) {
          delete charFreq[startChar]
        }
...
### Java
```java
public static Object nonRepeatSubstring(str) {
...
def nonRepeatSubstring(str):
    ...
    for windowEnd in range(len(str)):
        ...
        if endChar in charIndexMap:
            ...
            #is already ahead of the last index of
            #endChar, we'll keep windowStart
            windowStart = max(windowStart, charIndexMap[endChar] + 1)
...
auto nonRepeatSubstring(str) {
  ...
  for(auto windowEnd = 0; windowEnd < str.size(); windowEnd++) {
    ...
    if(endChar in charIndexMap) {
      ...
      //is already ahead of the last index of
      //endChar, we'll keep windowStart
      windowStart = std::max(windowStart, charIndexMap[endChar] + 1)
    }
...
### Java
```java
public static Object lengthOfLongestSubstring(str, k) {
  windowStart = 0
  maxLength = 0
...
### C++
```cpp
auto lengthOfLongestSubstring(str, k) {
  auto windowStart = 0
  auto maxLength = 0
  auto maxRepeatLetterCount = 0
  auto charFrequency = {}
  ...
  for(auto windowEnd = 0; windowEnd < str.size(); windowEnd++) {
    ...
    charFrequency[endChar]++
    //*REVIEW THIS LINE*
    maxRepeatLetterCount = std::max(maxRepeatLetterCount, charFrequency[endChar])
...
### Java
```java
public static Object lengthOfLongestSubstring (arr, k) {
  windowStart = 0
  maxLength = 0
  maxOnesCount = 0
...
### C++
```cpp
auto lengthOfLongestSubstring (arr, k) {
  auto windowStart = 0
  auto maxLength = 0
  auto maxOnesCount = 0
...
### Java
```java
public static Object findPermutation(str, pattern) {
  //sliding window
  windowStart = 0
  isMatch = 0
  charFrequency = {}
...
### C++
```cpp
auto findPermutation(str, pattern) {
  //sliding window
  auto windowStart = 0
  auto isMatch = 0
  auto charFrequency = {}
 ...
 for(i = 0; i < pattern.size(); i++) {
  ...
  for(windowEnd = 0; windowEnd < str.size(); windowEnd++) {
    ...
    if(isMatch === Object.keys(charFrequency).size()) {
      return true
    }
...
### Java
```java
public static Object findStringAnagrams(str, pattern){
  windowStart = 0, matched = 0, charFreq = {}
...
def findStringAnagrams(str, pattern):
    ...
    for windowEnd in range(len(str)):
        ...
        if matched === Object.keys(charFreq:.length):
            #have we found an anagram
            resultIndex.append(windowStart)
...
### C++
```cpp
auto findStringAnagrams(str, pattern){
  auto windowStart = 0, matched = 0, charFreq = {}
  ...
  for(auto windowEnd = 0; windowEnd < str.size(); windowEnd++) {
    ...
    if(matched === Object.keys(charFreq).size()){
      //have we found an anagram
      resultIndex.push_back(windowStart)
    }
...
### Java
```java
public static Object findSubstring(str, pattern) {
  windowStart = 0
  matched = 0
  substrStart = 0
...
### C++
```cpp
auto findSubstring(str, pattern) {
  auto windowStart = 0
  auto matched = 0
  auto substrStart = 0
  auto minLength = str.size() + 1
  auto charFreq = {}
...
### Java
```java
public static Object findWordConcatenation(str, words) {
...
def findWordConcatenation(str, words):
    if len(words) === 0 || words[0].length === 0:
        return []
    ...
    words.forEach((word) =>:
        ...
        for i in range((len(str) - wordCount * wordLength) + 1):
            wordsSeen = {}
...
### C++
```cpp
auto findWordConcatenation(str, words) {
  ...
  for(auto i = 0; i < (str.size() - wordCount * wordLength) + 1; i++) {
    auto wordsSeen = {}
    for(auto j = 0; j < wordCount; j++) {
      auto nextWordIndex = i + j * wordLength
      //get the next word from the string
      auto word = str.substring(nextWordIndex, nextWordIndex + wordLength)
      if(!(word in wordFreq)){
        //break if we don't need this word
        break
      }
